...
# 			SOFTANZA STRING LIBRARY (V0.9)
#		An advanced UNICODE-aware string library for Ring
#			 Part of the Softanza Framework

#---------------------------------------------------------------------------#
#									    #
# 	Description	: The core class for managing Unicode strings       #
#	Version		: V0.9 (July,2020)				    #
...
func QStringToString(oQStr)
	if IsQString(oQStr)
		...
		raise(stzStringError(:CanNotTransformQStringToString))
	ok
...
func StringsAreEqualCS(paStrings, pCaseSensitive)
	if NOT ListIsListOfStrings(paStrings)
		raise("You must provide a list of strings!")
	ok

	if NOT len(paStrings) > 1
		raise("You must provide at least two strings in the list!")
	ok
...
? cCode		eval(cCode)
	...
	// TODO: Reveview the following methods by reference to what provides
	// the stzConstraint class...
	...
	def CanNotBe(pcIsMethod)
		...
		if NOT This.IsEmpty()
			...
			return This.ObjectCanNotBe(pcIsMethod, :InObject = This)
		ok

	def MustHave(pcExpression)
		# The constraint is activated only if the string is not null

		if NOT This.IsEmpty()

			return This.ObjectMustHave(pcExpression, :InObject = This)
		ok

	def CanNotHave(pcExpression)
		# The constraint is activated only if the string is not null

		if NOT This.IsEmpty()

			return This.ObjectCanNotHave(pcExpression, :OnObject = This)
		ok
	...
	*/
	...
	def MarquersAndPositions()
		...
		*/
		...
		def MarquersAndPositionsQR(pcReturnType)
			...
			other
				raise("Insupported returned type!")
			off
	...
	def NumberOfOccurrenceCS(pcSubStr, pCaseSensitive)
		...
		oCopy = This.Copy()
		oCopy.RemoveAllCS(pcSubStr, pCaseSensitive)
	
		nResult = ( This.NumberOfChars() - oCopy.NumberOfChars() ) / StzStringQ(pcSubStr).NumberOfChars()
...
/*			
			...
			if This.IsBoundedByCS(pcSubstr1, pcSubstr2, pCaseSensitive)
				...
				# Define first and last bounds
	
				cFirstBound = ""
				cLastBound = ""
	
				if This.NFirstChars(oSubStr1.NumberOfChars()) = pcSubstr1
					cFirstBound = pcSubstr1
					cLastBound = pcSubstr2
	
				else
					cFirstBound = pcSubstr2
					cLastBound = pcSubstr1
				ok
	
				oFirstBound = new stzString(cFirstBound)
				oLastBound = new stzString(cLastBound)
	
				# Remove the bounds
	
				This.RemoveNFirstChars( oFirstBound.NumberOfChars() )
	...
	*/			
	...
	// Returns a subset of the string between nStartPos and nEndPos positions
	def Section(nStartPos, nEndPos)
		...
		if isNumber(nStartPos) and nStartPos > 0 and nEndPos = :EndOfLine
			return This.ForwardToEndOfLine( :StartingAt = nStartPos )
		ok

		if isNumber(nStartPos) and nStartPos > 0 and nEndPos = :EndOfWord
			return This.ToStzText().ForwardToEndOfWord( :StartingAt = nStartPos )
		ok
		...
		if nEndPos < 0
			nEndPos = -nEndPos
			nEndPos = This.NumberOfChars() - nEndPos + 1
		ok
		...
		return @oQString.mid( (nStartPos-1) , (nEndPos - nStartPos+1) )

		#< @FunctionFluentForm

		def SectionQ(nStartPos, nEndPos)
			return new stzString( This.Section(nStartPos, nEndPos) )

		#>

		#< @FunctionAlternativeForm

		def Slice(nStartPos, nEndPos)
			return This.Section(nStartPos, nEndPos)

			#< @FunctionFluentForm

			def SliceQ(nStartPos, nEndPos)
				return This.SectionQ(nStartPos, nEndPos)
	...
	// Extracts all the Sections enclosed between two sperators inside the string
	// --> Works recursively to extract any level of deep enclosings
	// --> Returns a list of strings
	def ExtractBetweenSubstringsCS(pSubstr1, pSubStr2, pCaseSensitive)
		/*
		Example:

		o1 = new stzString('[ "A", cStr, [ :hi1, :bye1 ], n, 5, obj, [ "hi2", "bye2" ], "C", aList ]')
		? o1.ExtractBetween("[", "]")
		*/

		if This.ContainsNo(pSubStr1) or This.ContainsNo(pSubStr2)
			return [ ]
		ok
		...
		oSep1 = new stzString(pSubStr1)
		nLenSep1 = oSep1.NumberOfChars()

		oCopy = This.Copy()

		bContinue = TRUE
		While bContinue
			n1 = oCopy.FindFirst(pSubStr1)
			n2 = oCopy.FindFirst(pSubStr2)
			
			cSection = oCopy.Section(n1+1, n2-1)
			if n1 > 0 and n2 > 0
				acResult + cSection
			else
				bContinue = FALSE
			...
			oCopy.Update( oCopy.Section(n2+1, :end ) )
		...
		def ExtractBetweenSubstringsCSQ(pSubStr1, pSubStr2, pCaseSensitive)
			return stzListOfStrings( This.ExtractBetweenSubstringsCS(pSubStr1, pSubStr2, pCaseSensitive) )
			
		def SectionsCS(pSubStr1, pSubStr2, pCaseSensitive)
			return This.ExtractBetweenSubstringsCS(pSubstr1, pSubStr2, pCaseSensitive)

			def SectionsCSQ(pSubStr1, pSubStr2, pCaseSensitive)
				return new stzListOfStrings( This.SectionsCS(pSubStr1, pSubStr2, pCaseSensitive) )

		def ExtractBetweenCS(pSubstr1, pSubStr2, pCaseSensitive)
			return This.ExtractBetweenSubstringsCS(pSubstr1, pSubStr2, pCaseSensitive)
	
			def ExtractBetweenCSQ(pSubstr1, pSubStr2, pCaseSensitive)
				return new stzString( This.ExtractBetweenCS(pSubstr1, pSubStr2, pCaseSensitive) )
	
	def ExtractBetweenSubstrings(pSubStr1, pSubStr2)
		/*
		NOTE: Currently this function extracts substrings in simple
		casesensitive like this:

		o1 = new stzString("amd[bmi]kmc")
		? o1.ExtractBetween("[","]") # --> "bmi"

		But it can not manage deep compbinations like this:
		...
		WARNING: if you use it now it would generate a stackoverflow!
		--> TODO: manage this by raising an error message!
		
		*/

		return This.ExtractBetweenSubstringsCS(pSubStr1, pSubStr2, :CaseSensitive = TRUE)

		def ExtractBetweenSubstringsQ(pSubStr1, pSubStr2)
			return stzListOfStrings( This.ExtractBetweenSubstrings(pSubStr1, pSubStr2) )

		def Sections(pSubStr1, pSubStr2)
			return This.ExtractBetweenSubstrings(pSubStr1, pSubStr2)

			def SectionsQ(pSubStr1, pSubStr2)
				return new stzListOfStrings( This.Sections(pSubStr1, pSubStr2) )

		def ExtractBetween(pSubstr1, pSubStr2)
			if BothAreNumbers(pSubstr1, pSubStr2)
				return This.Section(pSubstr1, pSubStr2)
	
			but BothAreStrings(pSubstr1, pSubStr2)
				return This.ExtractBetweenSubstrings(pSubstr1, pSubStr2)
			ok
	
			def ExtractBetweenQ(pSubstr1, pSubStr2)
				return new stzString( This.ExtractBetween(pSubstr1, pSubStr2) )
	...
	*/
		...
		*/
		...
		if NOT ( len(paOptions) = 0 or
			...
			if NOT oListOfProvidedOptions.IsMadeOfSome(aPossibleOptions)
				raise(stzStringError(:UnsupportedOptionsWhileInsertingListOfStrings))
			ok
		...
		def ReplaceAllCS(pSubStr, pNewSubStr, pCaseSensitive)
			...
			*/
			...
			if NOT ( cType = "NUMBER" or cType = "LIST" )
				raise(StzStringError(:IncorrectFormatOfCaseSensitiveParamList))
			ok
	...
	def StartsWithCS(pcSubStr, pCaseSensitive)
		...
		if NOT 	isNumber(pCaseSensitive) and
			...
			raise("Invalid param. pCaseSensitive must be 0 or 1 (TRUE or FALSE).")
	...
	def EndsWithCS(pcSubStr, pCaseSensitive)
		...
		if NOT 	isNumber(pCaseSensitive) and
			...
			raise("Invalid param. pCaseSensitive must be TRUE or FALSE (0 or 1).")
	...
	def FindNextOccurrencesCS(pcSubStr, pnStartingAt, pCaseSensitive)
		...
		def FindNextCS(pcSubStr, pnStartingAt, pCaseSensitive)
	...
	def FindNextOccurrences(pcSubStr, pnStartingAt)
		...
		def FindNext(pcSubStr, pnStartingAt)
	...
	def FindNthNextOccurrenceCS( n, pcSubStr, nStart, pCaseSensitive )
		...
		oString = This.SectionQ(nStart, :End)
		...
		def NextNthOccurrenceCS( n, pcSubStr, nStart, pCaseSensitive )
			return This.FindNthNextOccurrence( n, pcSubStr, nStart, pCaseSensitive )
	...
	def FindNthNextOccurrence( n, pcSubStr, nStart )
		...
		def FindNextNthOccurrence( n, pcSubStr, nStart )
			return This.FindNthNextOccurrence( n, pcSubStr, nStart, :CaseSensitive = TRUE )
	...
	def FindPreviousNthOccurrenceCS(n, pcSubStr, nStart, pCaseSensitive)
		...
		oString = This.SectionQ(1, nStart - 1)
		...
		def FindNthPreviousOccurrenceCS( n, pcSubStr, nStart, pCaseSensitive )
			return This.FindPreviousNthOccurrenceCS( n, pcSubStr, nStart, pCaseSensitive )
	...
	def FindPreviousNthOccurrence( n, pcSubStr, nStart )
		return This.FindPreviousNthOccurrenceCS( n, pcSubStr, nStart, :CaseSensitive = TRUE )

		def FindNthPreviousOccurrence( n, pcSubStr, nStart )
			return This.FindPreviousNthOccurrence( n, pcSubStr, nStart )
	...
	def VizFindAllXT(pcSubstr, paOptions) # TODO
		...
		if StzStringQ(pcSubStr).NumberOfChars() = 1
			...
			if StzListQ(paOptions).IsVizFindParamList()
				...
				raise(stzStringError(:IncorrectParameter))
			ok
...
*/	
		...
		def ContainsCS(cSubStr, pCaseSensitive) # :CaseSensitive = TRUE or :CaseSensitive = FALSE
			...
			if isNumber(pCaseSensitive) and
			   (pCaseSensitive = 0 or pCaseSensitive = 1)
				...
				raise("Error in param value! pCaseSensitive must be 0 or 1 (TRUE or FALSE).")
	...
	def SplitToPartsOfNCharsXT(n, pDirection)
		if n > this.NumberOfChars()
			raise("Can't proceed! Range of the part ("+ n +") must not exceed the lenght of the string ("+this.NumberOfChars()+").")
		ok
		...
		if ( pDirection = :Forward or pDirection = :Backward ) OR
		   ...
		   ( isList(pDirection) and
			...
			raise("Incorrect paDirection format!")
		ok
	...
	*/
	...
	def CharsAreSortedInDescending()
		...
		*/
		oTemp = new stzString( This.ReversedChars() )
	...
	def UnicodeCompareWithCS(pcOtherStr, pCaseSensitive)
		...
		if isNumber(pCaseSensitive) and
		   ( pCaseSensitive = 0 or pCaseSensitive = 1 )
			...
			raise("Incorrect value of pCaseSensitive! Should be 0 or 1 (TRUE or FALSE).")
		ok
	...
	def RemovePreviousOccurrence(pcSubStr, nStart, pcNewSubStr)
	  ...
	  #---------------------------------------------#
	 #    REMOVING LEFT OCCURRENCE OF SUBSTRING    # 
	#---------------------------------------------#
	...
	def RemoveLeftOccurrence(pcSubStr)
		This.RemoveLeftOccurrenceCS(pcSubStr, :CaseSensitive = TRUE)
	...
	*/
	...
	def IsNativeLanguageName() # Locale-specific
		...
		raise(stzStringError(:UnsupportedFeatureInThisVersion)) # TODO
	...
	def IsNativeCountryName() # Locale-specific
		...
		raise(stzString(:UnsupportedFeatureInThisVersion))
	...
	def IsMadeOfSomeOfTheseChars(acChars)
		if ListIsListOfChars(acChars)
			...
			raise("You must provide a list of chars!")
		ok
	...
	def IsCharInString()
		if This.NumberOfChars() = 3 and
		   (This.IsBoundedBy("'", "'") or
		   This.IsBoundedBy('"', '"'))
			return StzStringQ(This[2]).IsChar()
		else
			return FALSE
		ok
	...
	def IsLetter()
	  ...
	  #-------------------------------------------------#
	 #    COUNTING CHARS VERIFYING A GIVEN CONDITION   #
	#-------------------------------------------------#

	def CountCharsW(pcCondition)
		cCondition = StzStringQ(pcCondition).ReplaceAllCSQ("@Char", "@item", :CS = FALSE).Content()
		return StzListQ( This.ToListOfChars() ).CountItemsW(cCondition)
	...
	def BoxXT(paBoxOptions)
		...
		*/
		...
		but isList(paBoxOptions) and len(paBoxOptions) = 0
			...
			raise(stzStringError(:CanNotBoxTheString))
		ok
	...
	def IsItemOfThisList(paList)
		...
		if NOT isList(paList)
			raise("Invalid param type! You must provide a list.")
		ok
	...
	def SwapWith(pOtherStzStr)
		if IsStzString(pOtherStzStr)
			...
			raise(stzStringError(:CanNotSwapWithNonStzStringObject))
		ok
	...
	def Hash(pcHashingAlgo)
		...
		other
			raise("Unsupported hashing algorithm!")
		off
	...
	def CompressUsingBinary(cBinary)
		...
		if NOT oBinary.IsMadeOf(["0", "1"])
			raise(stzStringError(:CanNotCompressStringUsingBinary))
		ok
	...
	*/
				...
				*/
				...
				if NOT ( len(pValue[1]) = 3 AND
				   ...
				   isString(value) )
					...
					raise(stzStringError(:UnsupportedExpressionInOverloadedMinusOperator))
				ok
	...
	# Get defined params in list
	def pvtGetParamInList(pList,pParam)
		cResult = ""
