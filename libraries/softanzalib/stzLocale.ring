...
_aLocaleAbbreviationsXT = [
	...
	:Palestine = [ # The standard uses Palestinian Terrirories but we use Parestine
...
_cDefaultLocaleSeparator = "-"

_oDefaultLocale = new stzLocale("C")

_oActiveLocale = _oDefaultLocale

func StzLocaleQ(p)
	return new stzLocale(p)

func SetDefaultLocale(pLocaleAbbreviation)
	_oDefaultLocale = StzLocaleQ(pLocaleAbbreviation)

func DefaultLocale()
	return _oDefaultLocale

func SetActiveLocale(pLocale)
	_oActiveLocale = StzLocaleQ(pLocale)

func ActiveLocale()
	return _oActiveLocale
...
func LocaleCurrencySymbols()
	return _aLocaleCurrencySymbols
...
func SystemLocale()
	oSystemLocale = SystemLocaleAsStzObject()
	return oSystemLocale.Abbreviation()

func SystemLocaleAsStzObject()
	oQLocale = new QLocale("C")
	return new stzLocale(oQLocale.system())
...
// When locale abbreviation contains différent separators ("_" and "-")
// then takes the first one and unifies the abbreviation with it
func UnifiyLocaleAbbreviationSeparator(cLocaleAbbr)
	oStr = new stzString( cLocaleAbbr )
	
	if oStr.ContainsBoth( "_", "-" )
	
		cSep = ""
	
		if oStr.FindFirst("_") < oStr.FindFirst("-")
			oStr.ReplaceAll("-","_")
		else
			oStr.ReplaceAll("_","-")
		ok
...
func GetLocaleSeparator(cLocaleAbbr)

	if _@( cLocaleAbbr ).IsLocaleAbbreviation()
		if _@( cLocaleAbbr ).Contains("-")
			return "-"
		else
			return "_"
		ok
...
func DefaultLocaleSeparator()
	return _cDefaultLocaleSeparator

	func DefaultSeparatorForLocales()
		return DefaultLocaleSeparator()

func LocaleDefaultSeparator()
	return DefaultLocaleSeparator()

func SetDefaultLocaleSeparator(c)
	if c = "-" or c = "_"
...
class stzLocale from stzObject
	...
	@cSeparator = DefaultLocaleSeparator()
	...
	*/
	...
	def init(pLocale)
		...
		but IsString(pLocale)
			...
			but pLocale = :Default or pLocale = :DefaultLocale
				@oQLocale = DefaultLocale().QLocaleObject()
				@cAbbreviation = DefaultLocale().Abbreviation()
				return

			else
				if StringIsLocaleAbbreviation(pLocale)
			...
			@cSeparator = GetLocaleSeparator(pLocale)
		...
		but IsList(pLocale)
			...
			@oQLocale = new QLocale(cAbbr)
			@cAbbreviation = _( @oQLocale.name() ).Q.ReplaceQ( "-", :With = This.Separator() )
	...
	def Abbreviation()
		return _( @cAbbreviation ).Q.ReplaceManyQ( ["-","_"], :With = This.Separator() ).Content()
	...
	def Separator()
		...
		def AbbreviationSeparator()
			return This.Separator()

	def SetSeparator(cSep)
		if cSep = "-" or cSep = "_"
			@cSeparator = cSep
		ok

	def ApplyDefaultSeparator()
		This.SetSeparator( LocaleDefaultSeparator() )
	...
	def LanguageNativeName()
		//return StzLocaleQ([ :Language = This.LanguageName() ]).QLocaleObject().nativeLanguageName()
		return StzLocaleQ([ :Country = This.CountryName() ]).QLocaleObject().nativeLanguageName()
		
		//return @oQLocale.nativeLanguageName()
//? StzLocaleQ([ :Country = :South_Africa ]).LanguageNativeName()
