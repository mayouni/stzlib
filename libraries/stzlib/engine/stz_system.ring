...
if isWindows()
    $cStzSystemLib = currentdir() + "/zig-out/bin/stz_system.dll"
but isLinux()
    $cStzSystemLib = currentdir() + "/zig-out/lib/libstz_system.so"
but isMacOS()
    $cStzSystemLib = currentdir() + "/zig-out/lib/libstz_system.dylib"
ok
