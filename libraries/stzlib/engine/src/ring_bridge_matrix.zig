...
fn ring_MatrixAcoth(p: *anyopaque) callconv(.c) void {
    const ptr = matrix.stz_matrix_acoth(getMC(p, 1));
    if (ptr) |m| rcp(p, @ptrCast(m), MH) else rcp(p, @ptrFromInt(0), MH);
}
...
fn ring_MatrixCsc(p: *anyopaque) callconv(.c) void {
    const ptr = matrix.stz_matrix_csc(getMC(p, 1));
    if (ptr) |m| rcp(p, @ptrCast(m), MH) else rcp(p, @ptrFromInt(0), MH);
}
fn ring_MatrixSech(p: *anyopaque) callconv(.c) void {
    const ptr = matrix.stz_matrix_sech(getMC(p, 1));
    if (ptr) |m| rcp(p, @ptrCast(m), MH) else rcp(p, @ptrFromInt(0), MH);
}
fn ring_MatrixCsch(p: *anyopaque) callconv(.c) void {
    const ptr = matrix.stz_matrix_csch(getMC(p, 1));
    if (ptr) |m| rcp(p, @ptrCast(m), MH) else rcp(p, @ptrFromInt(0), MH);
}
fn ring_MatrixAsin(p: *anyopaque) callconv(.c) void {
    const ptr = matrix.stz_matrix_asin(getMC(p, 1));
    if (ptr) |m| rcp(p, @ptrCast(m), MH) else rcp(p, @ptrFromInt(0), MH);
}
fn ring_MatrixAcos(p: *anyopaque) callconv(.c) void {
    const ptr = matrix.stz_matrix_acos(getMC(p, 1));
    if (ptr) |m| rcp(p, @ptrCast(m), MH) else rcp(p, @ptrFromInt(0), MH);
}
fn ring_MatrixAsinh(p: *anyopaque) callconv(.c) void {
    const ptr = matrix.stz_matrix_asinh(getMC(p, 1));
    if (ptr) |m| rcp(p, @ptrCast(m), MH) else rcp(p, @ptrFromInt(0), MH);
}
fn ring_MatrixAcosh(p: *anyopaque) callconv(.c) void {
    const ptr = matrix.stz_matrix_acosh(getMC(p, 1));
    if (ptr) |m| rcp(p, @ptrCast(m), MH) else rcp(p, @ptrFromInt(0), MH);
}
fn ring_MatrixAtan(p: *anyopaque) callconv(.c) void {
    const ptr = matrix.stz_matrix_atan(getMC(p, 1));
    if (ptr) |m| rcp(p, @ptrCast(m), MH) else rcp(p, @ptrFromInt(0), MH);
}
fn ring_MatrixAtanh(p: *anyopaque) callconv(.c) void {
    const ptr = matrix.stz_matrix_atanh(getMC(p, 1));
    if (ptr) |m| rcp(p, @ptrCast(m), MH) else rcp(p, @ptrFromInt(0), MH);
}
fn ring_MatrixTan(p: *anyopaque) callconv(.c) void {
    const ptr = matrix.stz_matrix_tan(getMC(p, 1));
    if (ptr) |m| rcp(p, @ptrCast(m), MH) else rcp(p, @ptrFromInt(0), MH);
}
fn ring_MatrixTanh(p: *anyopaque) callconv(.c) void {
    const ptr = matrix.stz_matrix_tanh(getMC(p, 1));
    if (ptr) |m| rcp(p, @ptrCast(m), MH) else rcp(p, @ptrFromInt(0), MH);
}
fn ring_MatrixSinh(p: *anyopaque) callconv(.c) void {
    const ptr = matrix.stz_matrix_sinh(getMC(p, 1));
    if (ptr) |m| rcp(p, @ptrCast(m), MH) else rcp(p, @ptrFromInt(0), MH);
}
fn ring_MatrixCosh(p: *anyopaque) callconv(.c) void {
    const ptr = matrix.stz_matrix_cosh(getMC(p, 1));
    if (ptr) |m| rcp(p, @ptrCast(m), MH) else rcp(p, @ptrFromInt(0), MH);
}
fn ring_MatrixSin(p: *anyopaque) callconv(.c) void {
    const ptr = matrix.stz_matrix_sin(getMC(p, 1));
    if (ptr) |m| rcp(p, @ptrCast(m), MH) else rcp(p, @ptrFromInt(0), MH);
}
fn ring_MatrixCos(p: *anyopaque) callconv(.c) void {
    const ptr = matrix.stz_matrix_cos(getMC(p, 1));
    if (ptr) |m| rcp(p, @ptrCast(m), MH) else rcp(p, @ptrFromInt(0), MH);
}
fn ring_MatrixLog(p: *anyopaque) callconv(.c) void {
    const ptr = matrix.stz_matrix_log(getMC(p, 1));
    if (ptr) |m| rcp(p, @ptrCast(m), MH) else rcp(p, @ptrFromInt(0), MH);
}
fn ring_PowerGeneral(p: *anyopaque) callconv(.c) void {
    const ptr = matrix.stz_matrix_power_general(getMC(p, 1), g(p, 2));
    if (ptr) |m| rcp(p, @ptrCast(m), MH) else rcp(p, @ptrFromInt(0), MH);
}
...
fn ring_SchurT(p: *anyopaque) callconv(.c) void {
    const ptr = matrix.stz_matrix_schur_t(getMC(p, 1));
    if (ptr) |m| rcp(p, @ptrCast(m), MH) else rcp(p, @ptrFromInt(0), MH);
}
fn ring_SchurInverse(p: *anyopaque) callconv(.c) void {
    const ptr = matrix.stz_matrix_schur_inverse(getMC(p, 1));
    if (ptr) |m| rcp(p, @ptrCast(m), MH) else rcp(p, @ptrFromInt(0), MH);
}
...
pub fn ringlib_init(p: *anyopaque) callconv(.c) void {
    const funcs = [_]R.Reg{
        ...
        .{ .name = "stzengine" ++ "matrixlog", .func = &ring_MatrixLog },
        .{ .name = "stzengine" ++ "matrixsin", .func = &ring_MatrixSin },
        .{ .name = "stzengine" ++ "matrixsinh", .func = &ring_MatrixSinh },
        ...
        .{ .name = "stzengine" ++ "matrixcsch", .func = &ring_MatrixCsch },
        .{ .name = "stzengine" ++ "matrixacos", .func = &ring_MatrixAcos },
        .{ .name = "stzengine" ++ "matrixasinh", .func = &ring_MatrixAsinh },
        .{ .name = "stzengine" ++ "matrixacosh", .func = &ring_MatrixAcosh },
        .{ .name = "stzengine" ++ "matrixatanh", .func = &ring_MatrixAtanh },
        .{ .name = "stzengine" ++ "matrixtanh", .func = &ring_MatrixTanh },
        .{ .name = "stzengine" ++ "matrixcosh", .func = &ring_MatrixCosh },
        .{ .name = "stzengine" ++ "matrixcos", .func = &ring_MatrixCos },
        .{ .name = "stzengine" ++ "matrixpowergeneral", .func = &ring_PowerGeneral },
