...
fn ring_StringEditCluster(p: *anyopaque) callconv(.c) void {
    ...
    const cs: c_int = @intFromFloat(ring_vm_api_getnumber(p, 3));
    ring_vm_api_retcpointer(p, @ptrCast(string.str_edit_cluster(h, thr, cs)), FIND_HANDLE);
}

fn ring_StringSentenceStat(p: *anyopaque) callconv(.c) void {
    const h = getHandle(p, 1);
    const mode: c_int = @intFromFloat(ring_vm_api_getnumber(p, 2));
    ring_vm_api_retnumber(p, @floatFromInt(string.str_sentence_stat(h, mode)));
}
...
fn ring_StringCountWordCS(p: *anyopaque) callconv(.c) void {
    ...
    const wlen: usize = @intCast(ring_vm_api_getstringsize(p, 2));
    const cs: c_int = @intFromFloat(ring_vm_api_getnumber(p, 3));
    ring_vm_api_retnumber(p, @floatFromInt(string.str_count_word_cs(h, w, wlen, cs)));
}
...
fn ring_StringWordNgramFreq(p: *anyopaque) callconv(.c) void {
    const h = getHandle(p, 1);
    const ng: c_int = @intFromFloat(ring_vm_api_getnumber(p, 2));
    const cs: c_int = @intFromFloat(ring_vm_api_getnumber(p, 3));
    const nTop: c_int = @intFromFloat(ring_vm_api_getnumber(p, 4));
    R.retHandle(p, @ptrCast(string.str_word_ngram_freq(h, ng, cs, nTop)));
}
...
fn ring_StringSearchTokens(p: *anyopaque) callconv(.c) void {
    const h = getHandle(p, 1);
...
const regs = [_]R.Reg{
    ...
    .{ .name = "stzenginestringscriptnameslist", .func = &ring_StringScriptNamesList },
