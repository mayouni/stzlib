...
pub export fn str_all_substrings(handle: ?*StzString) callconv(.c) ?*StzString {
    ...
    var off: usize = 0;
    var idx: usize = 0;
    while (off < src.len and idx < cp_count) {
        offsets[idx] = off;
    ...
    // Pre-allocate: rough estimate n*(n+1)/2 * avg_len + delimiters
    ...
    for (0..cp_count) |i| {
        for (i + 1..cp_count + 1) |j| {
            if (!first) {
                result.data.append(gpa, 0) catch { // \x00 delimiter
                    result.deinit();
                    gpa.destroy(result);
                    return null;
                };
            }
            result.data.appendSlice(gpa, src[offsets[i]..offsets[j]]) catch {
                result.deinit();
                gpa.destroy(result);
                return null;
            };
...
/// Return all UNIQUE substrings joined by \x00 delimiter.
/// Uses a hash set for O(1) dedup per substring.
pub export fn str_all_substrings_unique(handle: ?*StzString) callconv(.c) ?*StzString {
    ...
    // Hash set for dedup (key = byte slice content hash)
    const HashCtx = struct {
        slices: []const u8,
        starts: []const u32,
        lens: []const u32,

        pub fn hash(ctx: @This(), key: usize) u64 {
            const start = ctx.starts[key];
            const len = ctx.lens[key];
            return std.hash.Wyhash.hash(0, ctx.slices[start .. start + len]);
        }
        pub fn eql(ctx: @This(), a: usize, b: usize) bool {
            const sa = ctx.slices[ctx.starts[a] .. ctx.starts[a] + ctx.lens[a]];
            const sb = ctx.slices[ctx.starts[b] .. ctx.starts[b] + ctx.lens[b]];
            return mem.eql(u8, sa, sb);
        }
    };
    _ = HashCtx;

    // Simpler approach: use a StringHashMap for seen substrings
...
/// Return unique chars as \x00-delimited string (case-insensitive).
/// Folds case before dedup. Returns lowercased unique chars.
pub export fn str_unique_chars_ci(handle: ?*StzString) callconv(.c) ?*StzString {
    ...
    while (i < src.len) {
        ...
        const folded: i32 = if (unicode.stz_unicode_is_upper(cp_val) == 1)
        ...
        if (!seen.contains(folded)) {
            ...
            // Encode the folded (lowercased) codepoint
            var buf: [4]u8 = undefined;
            const folded_u21: u21 = @intCast(@as(u32, @intCast(folded)));
...
test "format: unique_chars_ci" {
    const s = str_from("AaBbCc", 6);
    defer core.str_free(s);
    const r = str_unique_chars_ci(s);
    defer core.str_free(r);
    try std.testing.expect(r != null);
