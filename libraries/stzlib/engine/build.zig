...
const base_domains = [_]Domain{
    ...
    // needs_wgpu: GS6a routes t-SNE epochs through its own wgpu device (tsne_gpu.zig), the neural precedent
    .{ .name = "stz_stats", .entry = "src/stz_stats_entry.zig", .needs_ring = true, .needs_utf8proc = true, .needs_wgpu = true },
    ...
    .{ .name = "stz_seclog", .entry = "src/stz_seclog_entry.zig", .needs_ring = true },
...
fn addTreeSitter(mod: *std.Build.Module, lib: *std.Build.Step.Compile, b: *std.Build) void {
    ...
    lib.addCSourceFiles(.{
        ...
        // UBSan OFF for this one vendored library, unlike pcre2 / utf8proc /
        // sqlite above which opt IN to trap mode.
        //
        // Once the default optimize mode became ReleaseSafe, Zig's implicit C
        // sanitiser started trapping here: ANY tree-sitter parse -- `class A {}`
        // is enough -- died with SIGILL and no message (a bare ud2, which is what
        // trap mode emits, as against a Zig safety panic which prints). Debug and
        // ReleaseFast were both fine, which is what identified it as a sanitiser
        // trap rather than a miscompile.
        //
        // These parser.c files are MACHINE-GENERATED by tree-sitter's codegen and
        // are not ours to correct; the runtime does deliberate unaligned and
...
pub fn build(b: *std.Build) void {
    ...
    // THE DEFAULT IS ReleaseSafe, NOT Debug.
    //
    // `standardOptimizeOption(.{})` defaults to Debug, and every documented build
    // command in this repo is a bare `zig build` -- so every DLL the library has
    // ever loaded was compiled UNOPTIMISED. The DLLs are not tracked in git, so
    // this was true for every developer and every deploy, not just one machine.
    //
    // Measured on 2M engine-resident f64s, twenty passes each:
    //
    //                    Debug     ReleaseSafe
    ...
    //
    // On string / list / matrix work the same change buys only ~15%, because
    // there the marshalling dominates and the engine loop is a small share of the
    // time. That contrast is the point: PHASE 3 MADE COMPUTE RESIDENT, so from
    // here on the engine loop IS the cost, and how it is compiled starts to
    // matter. Fixing this before writing a single SIMD kernel is the difference
    // between optimising code and optimising code that was never compiled.
    //
    // ReleaseSafe rather than ReleaseFast because it measured the SAME on every
    // kernel above while keeping bounds, overflow and alignment checks. This
    // engine is called across a C ABI by a dynamically typed interpreter that can
    // hand it any index; a clean panic beats silent memory corruption, and here
    // it costs nothing. The fuzz harnesses below already made that argument.
    //
    // `-Doptimize=Debug` still works for debugging.
    //
    // Declared with b.option rather than standardOptimizeOption: passing
    // `.preferred_optimize_mode` to the latter does NOT change the default, it
    // replaces `-Doptimize=<mode>` with a `-Drelease` boolean and still leaves
    // Debug in force unless that flag is given. Checked with `zig build --help`.
    const optimize = b.option(
        std.builtin.OptimizeMode,
        "optimize",
        "Optimization mode (default: ReleaseSafe -- see the note above)",
    ) orelse .ReleaseSafe;
    ...
    // (imported), only the curated exports are callable. `zig build wasm`.
    {
        ...
        const wasm_groups = b.option([]const u8, "wasm-groups", "engine groups in stz.wasm (comma-sep: solver,aggregation,numtheory,pattern,graph,gpu,sound)") orelse "solver,aggregation,numtheory,pattern,graph,gpu,sound";
