...
pub const milestones = [_]Milestone{
    ...
    .{
        ...
        .status = .partial,
        .summary = "Session 38-41: 7 regression suites pin the 47 latent bugs found+fixed during M-S1 Phase 2 (183 assertions: listoflists, hashlist, submodules1/2/3/4, show-recursion-stress). Session 41 adds Show recursion stress test exercising 10-level nested input, mutually-recursive NL chain, and repeated-call sibling clobbering -- confirms the M-S1 Phase 2 var rename was sufficient (Ring func locals are NOT actually shared across recursive call frames despite the global-funcs-are-global warning). Also documented: Ring's `a = [i, a]` literal doesn't deep-copy the inner reference, requires explicit copy. ARCHITECTURAL: stz_value.dll/stz_list.dll cross-DLL handle issue persists -- engine fast-paths in Replacer/Remover stay disabled until engine session lands a shared handle table or string-only variants. Function-form tests, narrated GIVEN/WHEN/THEN, test runner integration still pending",
    },
    .{
        .id = "M-S3",
        .track = "stzlib",
