...
pub const submodules = [_]Submodule{
    ...
    .{ .parent = "list", .name = "stzListOfLists",        .methods = 55, .engine_backed = 0,  .scoping_clean = false, .notes = "scoping cleanup in progress: init, Add*, NthList, Find*Lists, FindMissingItems, NumberOfMissingItems, ItemsAtPositionN, FindSmallest/LargestList, *ListZ, SizeOf* done; 3 latent bugs fixed (AddMany loop no-op, NthList :Last fall-through, FindManyInListsCS dedup discarded)" },
};
