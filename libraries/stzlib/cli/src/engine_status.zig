...
pub const milestones = [_]Milestone{
    ...
    .{
        ...
        .status = .partial,
        .summary = "Sessions 38-46: 19 regression suites pin 52 latent bugs, 442 cumulative assertions. Session 43 shipped the cross-DLL handle fix for list/value (stz_list_replace_all_string_cs / remove_all_string_cs / set_string). Session 44 added pure-Ring fallbacks for stzYielder + test_edge_cases (37) + test_function_forms (31). Session 45 closed the M-E3 regression slice: test_number_regression (40) + test_table_regression (24) + test_graph_regression (21) -- 4 latent bugs (stzString Represents*Number family + LeftChar/RightChar lost in modularization; stzTable Section page-reading sweep; stzGraph HasEdge param typo). Session 46 SHIPPED the engine-side direct-marshal yielder bridge: 7 new C ABI variants (Map/MapIndexed/Filter/FilterMap/Reduce/CountWhere/ReduceConcat *Direct) take Ring lists directly, marshal-op-unmarshal inside the yielder DLL with no handle table involved. Ring write-side externs (newlist/retlist/add{int,double,string,newlist}) added to ring_api.zig. Key learning: regular `pub fn` Zig calls from inside a callconv(.c) bridge fn crash the DLL on Windows (calling-convention mismatch) -- route everything through C ABI exports. stzYielder pure-Ring fallbacks removed. Still pending: function-form tests across remaining submodules, narrated GIVEN/WHEN/THEN runner",
    },
    .{
        .id = "M-S3",
        .track = "stzlib",
