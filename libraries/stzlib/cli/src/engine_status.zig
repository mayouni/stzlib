...
pub const submodules = [_]Submodule{
    ...
    .{ .parent = "list", .name = "stzListOfLists",        .methods = 55, .engine_backed = 0,  .scoping_clean = false, .notes = "~90 methods cleaned through batch 3 (adds Entry*/Contains*/EachListContains*/Merged/SizeOfEach@Is/CommonItemsCS/Cols/NthColumn(XT)/AddCol(XT)/SortNthList/SortDownNthList/IsSortedIn*On/Sort/Sorted/SortDown/SortedDown/SortOn). 19 latent bugs fixed across stzListOfLists batches (incl. SizeOfEach@Is always returned 1, AddCol used undefined nLen, NthListSorted called nonexistent method, IsSortedDownOn called nonexistent IsSortedInDescendingDown, NumberOfOccurrenceOfEntry/HowMany* used undefined o1)" },
};
