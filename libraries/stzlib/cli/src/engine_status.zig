...
pub const milestones = [_]Milestone{
    ...
    .{
        ...
        .status = .partial,
        .summary = "Sessions 38-47: 19 regression suites pin 64 latent bugs, ~615 cumulative assertions. Session 43 shipped the cross-DLL handle fix for list/value. Session 44 added pure-Ring fallbacks for stzYielder + edges/forms suites. Session 45 closed the M-E3 slice (Number/Table/Graph regressions). Session 46 SHIPPED the engine-side direct-marshal yielder bridge (7 C ABI variants take Ring lists directly; calling-convention key learning documented). Session 47 broad regression sweep -- 8 new domain suites (regex, json, csv, datetime, matrix, calendar, listofnumbers, plus shadow audit) finding 12 more latent bugs: case-insensitive shadow family closed (insert/reverse), modularization gaps closed (Represents*Number family + IsNumberInString/IsListInString + LeftChar/RightChar), engine reg typo (stzenginecsvIsvalid -> lowercase), DaysTo direction bug propagating into TotalDays/TotalWeeks, matrix dim-pair ctor unreachable + invalid list(n,v), CSV alias wrong target + ignored cSep param, stray debug print in IsListOfNumbers polluting stdout, active-form-silently-no-mutate in SortInDescending. New bug pattern catalogued: 'active form returns without mutating'. Still pending: same audit on more domains; narrated GIVEN/WHEN/THEN runner; reactive async harness",
    },
    .{
        .id = "M-S3",
        .track = "stzlib",
