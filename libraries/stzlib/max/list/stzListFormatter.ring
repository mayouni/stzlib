# Functions that generate a stringified,
# readbale, visual representation of a list

$MinValueForComputableShortFormXT = 10
$bCheckForListExpansionXT = TRUE # Always TRUE, except with @@()

#---

func MinValueForComputableShortFormXT()
	return $MinValueForComputableShortFormXT
...
func SetValueForComputableShortFormXT(n)
	...
	$MinValueForComputableShortFormXT = n
...
func ComputableForm(pValue)
	...
	$bCheckForListExpansionXT = FALSE
	cResult = ComputableFormXT(pValue, " ", "")
	$bCheckForListExpansionXT = TRUE
...
# Determine if a list should be displayed inline or expanded
...
#-----------------------------------------#
#  THE CORE FUNCTION FOR FORMATTING LISTS #
#-----------------------------------------#
	...
	but isList(pValue)
		...
		for i = 1 to nLen
			...
			but isList(aContent[i])
				...
				if $bCheckForListExpansionXT and ShouldExpandList(aContent[i])

				# We need to make sure each level's brackets line up vertically
...
# A helper function to handle nested list formatting with proper alignment
...
func FormatNestedList(aList, cCurrentIndent)
	...
	# The closing bracket should be at the same indentation
	# level as the opening one
