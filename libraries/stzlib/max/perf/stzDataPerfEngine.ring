...
class stzDataPerfEngine from stzObject
    ...
    def SetActivePlan(cPlanName)
        @active_plan = cPlanName
        return This
    
    def GetActivePlan()
        return @active_plan
    
    def GetRulesForPlan(cPlanName)
        nLen = len(@rule_sets)
        for i = 1 to nLen
            if @rule_sets[i][:name] = cPlanName
                return @rule_sets[i][:plan][:rules]
            ok
        next
        return []
    
    def GetActiveRules()
        return This.GetRulesForPlan(@active_plan)
    
    def GetPlanContext(cPlanName)
        nLen = len(@rule_sets)
        for i = 1 to nLen
            if @rule_sets[i][:name] = cPlanName
                return @rule_sets[i][:plan][:context]
            ok
        next
    ...
    def EvaluateRule(aRule, aModelData)
        ...
        on "large_text_fields"
            return This.CheckLargeTextFields(aModelData)
        on "frequent_updates"
    ...
    def CheckFrequentUpdates(aModelData)
    ...
    def SetThreshold(cThresholdName, nValue)
        @thresholds[cThresholdName] = nValue
        return This
