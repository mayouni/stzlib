...
oModel {
    ...
    ? @@NL(Relations())  # --> [[["type", "one-to-many"], ["from", "users"], ["to", "posts"], ["field", "user_id"], ["inferred", 1]]]
    try
        AddRelation("one-to-many", "users", "posts", [ :field = "user_id" ])
    catch
        ? "Error: Can't define relations when inference is active!"  # --> Error message
    done
}
...
oModel {
    ...
    aResult = Validate()
    ? aResult[:errors_count]  # --> 2
    ? aResult[:fixes_applied]  # --> 2
    ? TableNames()  # --> ["unnamed_table_1"]
    ? TableFields("unnamed_table_1")  # --> [[ :name = "id", :type = "string" ], [ :name = "name", :type = "string" ], [ :name = "name_1", :type = "string" ]]
}
...
oModel {
    ...
    ? oNewModel.TableExists("users")  # --> TRUE
}
