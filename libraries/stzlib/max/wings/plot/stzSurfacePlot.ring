#---------------------------------------#
#  SURFACE COMPOSITION-ORIENTED PLOT   #
#---------------------------------------#
...
def init(paData)
	if NOT isList(paData)
		raise("Can't create the stzSquarePlot object! paData must be a list.")
	ok

	if NOT (IsListOfNumbers(paData) or IsHashList(paData))
		raise("Can't create the stzSquarePlot object! paData must be a list of numbers or a hashlist.")
	ok
