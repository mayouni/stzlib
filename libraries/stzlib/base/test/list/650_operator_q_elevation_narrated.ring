...
Scenario("stzList (/): divide into N parts -- raw stays raw, Q() elevates")
	Given("the list [1, 2, 3, 4, 5, 6]")
	Then("/ 3 returns a RAW list", isList( Q([1,2,3,4,5,6]) / 3 ), TRUE)
	Then("/ 3 divides into 3 equal parts", ListEq( Q([1,2,3,4,5,6]) / 3, [[1,2],[3,4],[5,6]] ), TRUE)
	Then("uneven divides with a front-loaded remainder", ListEq( Q([1,2,3,4,5,6,7]) / 3, [[1,2,3],[4,5],[6,7]] ), TRUE)
...
Scenario("stzString (*) and (/): same rule")
	...
	Then("/ n divides into n equal parts (same as lists)", ListEq( Q("RingRingRing") / 3, ["Ring","Ring","Ring"] ), TRUE)
