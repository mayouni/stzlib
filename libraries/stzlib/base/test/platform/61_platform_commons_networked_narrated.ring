...
Scenario("POST /session mints a token for good credentials only")
	...
	Then("the body refuses with 401", StzFindFirst("401", r2) > 0, TRUE)
EndScenario()
...
Scenario("GET /whoami resolves the caller behind a token")
	...
	Then("the body names her", StzFindFirst("alice", r) > 0, TRUE)
	When("an unknown token is presented")
	r2 = HttpGet(oClient, nPort, oP, "/whoami?token=sess_bogus")
	Then("it is refused with 401", StzFindFirst("401", r2) > 0, TRUE)
EndScenario()
...
Scenario("POST /message carries mail, authed as the sender")
	...
	Then("the message is accepted", StzFindFirst("posted", r) > 0, TRUE)
	...
	Then("alice's message is really there",
		StzFindFirst("alice", r2) > 0 and StzFindFirst("hi", r2) > 0, TRUE)
EndScenario()
...
Scenario("an inbox is private -- the token must match the owner")
	...
	Then("the body refuses with 401", StzFindFirst("401", r) > 0, TRUE)
EndScenario()
...
Scenario("the key/value store is session-gated and shared")
	...
	Then("the value persisted in the shared store", StzFindFirst("dark", r) > 0, TRUE)
