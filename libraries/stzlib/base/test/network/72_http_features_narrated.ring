...
Scenario("The options blob reflects the client settings")
    ...
    Then("it carries the credentials", StzFindFirst(cBlob, "userpwd=alice:secret") > 0, TRUE)
    Then("it carries the auth type", StzFindFirst(cBlob, "authtype=digest") > 0, TRUE)
    Then("it carries the proxy", StzFindFirst(cBlob, "proxy=http://127.0.0.1:8080") > 0, TRUE)
    Then("it carries the client cert", StzFindFirst(cBlob, "sslcert=client.pem") > 0, TRUE)
    Then("it disables verification", StzFindFirst(cBlob, "verifyssl=0") > 0, TRUE)
    Then("it disables redirects", StzFindFirst(cBlob, "followredirects=0") > 0, TRUE)
EndScenario()
...
Scenario("A bearer-token client emits a bearer option")
    ...
    Then("the blob carries the bearer token",
        StzFindFirst(o2._ComposeOptionsBlob(), "bearer=tok-12345") > 0, TRUE)
EndScenario()
...
Scenario("Response headers are captured")
    ...
    Then("they include a Content-Type header",
        StzFindFirst(lower(cHdrs), "content-type") > 0, TRUE)
EndScenario()
...
Scenario("gzip responses are transparently decompressed (vendored zlib)")
    ...
    Then("the body was decompressed to readable JSON",
        StzFindFirst(o5.ResponseBody(), "gzipped") > 0, TRUE)
EndScenario()
