...
Scenario("attach the rule to the graph, then ask the GRAPH (the idiomatic path)")
	Given("a fluently-declared rule")
	oRule = new stzGraphRule("no-llm-effectful")
	oRule.SetDomainQ("agentic")
	oRule.WhenQ("kind", "equals", "llm_actor")
	oRule.WhenQ("capabilities", "contains", "effectful")
	...
	When("it is attached to the graph and the GRAPH is checked")
	$oG.AddRule(oRule)              # the rule becomes part of the graph's logic
	aF = $oG.CheckRules()          # the scope stays on the graph, never inverts
	...
	Then("the graph is not sound while a finding stands", $oG.RulesAreSound(), FALSE)
	$oG.ClearAttachedRules()       # leave $oG clean for the object-level tests below
EndScenario()
...
Scenario("the operator set covers what the code/agent phases need")
	...
	# each operator is attached to the graph in turn, and the GRAPH is checked
	When("not-equals selects the complement")
	oG2.AddRule(StzGraphRuleQ("not-step").WhenQ("kind", "not-equals", "step").ThenViolationQ("x"))
	Then("only the decision node matches", len(oG2.CheckRules()), 1)
	oG2.ClearAttachedRules()

	When("missing flags nodes lacking a property")
	oG2.AddRule(StzGraphRuleQ("no-assignee").WhenQ("assignee", "missing", "").ThenViolationQ("x"))
	Then("the two nodes with no assignee are flagged", len(oG2.CheckRules()), 2)
	oG2.ClearAttachedRules()

	When("exists flags nodes that have it")
	oG2.AddRule(StzGraphRuleQ("has-assignee").WhenQ("assignee", "exists", "").ThenViolationQ("x"))
	Then("only the assigned node matches", len(oG2.CheckRules()), 1)
	oG2.ClearAttachedRules()

	When("equals is case-insensitive (matching the governance idiom)")
	oG2.AddRule(StzGraphRuleQ("STEP-upper").WhenQ("kind", "equals", "STEP").ThenViolationQ("x"))
	Then("STEP matches step", len(oG2.CheckRules()), 2)
	oG2.ClearAttachedRules()
EndScenario()
...
Scenario("a rule with no clauses holds vacuously; bad inputs are refused early")
	$oG.AddRule(new stzGraphRule("empty"))
	Then("no clauses => no findings", len($oG.CheckRules()), 0)
	$oG.ClearAttachedRules()
