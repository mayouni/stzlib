...
Scenario("STAGE 3: a cross-point derivation rule rejects and rolls back")
	...
	Then("the why names the derivation stage", StzFindFirst("derivation", r2[:why]) = 1, TRUE)
...
Scenario("STAGE 4: governance gates the refinement before any mutation")
	...
	$o2.SetGovernedBy(new stzGovernance("release-ops")).SetActor("release-bot")
	# config THROUGH the code so its one live governance copy is the truth
	$o2.SetRiskFor("sort", 3).SetAllowRefine("sort").SetAuthorityLevel(:Delegated)
...
Scenario("an undeclared-risk point cannot be refined under governance")
	...
	$o3.SetGovernedBy(new stzGovernance("release-ops-2")).SetActor("bot")
	$o3.SetAllowRefine("vat").SetAuthorityLevel(:Autonomous)   # but no SetRiskFor("vat")
...
Scenario("all four stages compose: structural, constraint, derivation, governance")
	...
	$o4.SetGovernedBy(new stzGovernance("full")).SetActor("op")
	$o4.SetRiskFor("vat", 1).SetAllowRefine("vat").SetAuthorityLevel(:Delegated)
