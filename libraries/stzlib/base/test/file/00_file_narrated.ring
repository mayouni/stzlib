...
Scenario("FileCreate writes a new file with read-as-you-go")
    ...
    Then("both lines are present", StzFindFirst("beta", oCreator.Content()) > 0, TRUE)
    oCreator.Close()
EndScenario()
...
Scenario("FileAppend creates-then-appends a missing log (append-or-create)")
    ...
    Then("the appended content is present", StzFindFirst("first", oLog.Content()) > 0, TRUE)
...
Scenario("FileOverwrite reads the original before replacing")
    ...
    Then("the new content replaced the old", StzFindFirst("Completed", oWriter.Content()) > 0, TRUE)
    Then("the old content is gone", StzFindFirst("old line one", oWriter.Content()), 0)
    oWriter.Close()
EndScenario()
...
Scenario("FileUpdate makes targeted edits")
    ...
    Then("the replacement took effect", StzFindFirst("Host=newserver", oUpdater.Content()) > 0, TRUE)
    Then("the inserted line is present", StzFindFirst("New=Default", oUpdater.Content()) > 0, TRUE)
    Then("the removed line is gone", StzFindFirst("Obsolete", oUpdater.Content()), 0)
    oUpdater.Close()
EndScenario()
