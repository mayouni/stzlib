...
Scenario("FileCreate writes a new file with read-as-you-go")
    ...
    Then("both lines are present", StzFindFirst(oCreator.Content(), "beta") > 0, TRUE)
    oCreator.Close()
EndScenario()
...
Scenario("FileAppend creates-then-appends a missing log (append-or-create)")
    ...
    Then("the appended content is present", StzFindFirst(oLog.Content(), "first") > 0, TRUE)
...
Scenario("FileOverwrite reads the original before replacing")
    ...
    Then("the new content replaced the old", StzFindFirst(oWriter.Content(), "Completed") > 0, TRUE)
    Then("the old content is gone", StzFindFirst(oWriter.Content(), "old line one"), 0)
    oWriter.Close()
EndScenario()
...
Scenario("FileUpdate makes targeted edits")
    ...
    Then("the replacement took effect", StzFindFirst(oUpdater.Content(), "Host=newserver") > 0, TRUE)
    Then("the inserted line is present", StzFindFirst(oUpdater.Content(), "New=Default") > 0, TRUE)
    Then("the removed line is gone", StzFindFirst(oUpdater.Content(), "Obsolete"), 0)
    oUpdater.Close()
EndScenario()
