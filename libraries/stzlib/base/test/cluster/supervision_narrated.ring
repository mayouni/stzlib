...
Scenario("AUTOSCALE POLICY (pure): decisions from real metrics")
	...
	$oSup.Policy(:graph, 1, 3).WithWaterMarks(0.25, 0.75)
...
Scenario("REAL health: a live fleet reports healthy; supervision heals via spawn")
	...
	$oHeal.WithFacet(:knowledge, 1).WithWorkerTTL(15000).WithBasePort($nBase)
	$oHeal.Start()
...
Scenario("REAL elastic scale + graceful drain")
	...
	$oEl.WithFacet(:graph, 1).WithWorkerTTL(15000).WithBasePort($nBase + 40)
	$oEl.Start()
	$oEl.WaitReady(20000)
...
Scenario("R5 COMPOSITION: the supervisor runs as a supervised reactor job")
	...
	$oJob.WithFacet(:graph, 1).WithWorkerTTL(15000).WithBasePort($nBase + 80)
	$oJob.Start()
