...
Scenario("BREAKER: consecutive failures OPEN a worker's circuit")
	...
	$oB.SetCircuitBreaker(2, 60000)                  # opens after 2 fails, 60s cooldown
...
Scenario("FAILOVER: a route tries multiple workers before giving up")
	...
	oF.SetCircuitBreaker(99, 60000)   # keep circuits closed so all are tried
...
Scenario("SELF-HEAL: RestartDead clears a worker's tripped circuit")
	...
	oSH.WithNLP(1).SetCircuitBreaker(1, 60000).SetWorkerTTL(8000)
...
Scenario("LIVE: a broken sibling never breaks the request (failover + isolation)")
	...
	$oL.WithNLP(1).SetWorkerTTL(15000).SetBasePort($nBase)
	$oL.SetCircuitBreaker(2, 60000).SetMaxTries(3)
