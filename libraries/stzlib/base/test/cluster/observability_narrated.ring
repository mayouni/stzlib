...
Scenario("trace ids are unique and monotonic")
	...
	Then("ids carry the trace prefix", StzFindFirst(c1, "t-") = 1, TRUE)
EndScenario()
...
Scenario("Route assigns a trace and records the request even when it declines")
	...
	Then("a trace id was still assigned", StzFindFirst($oC.LastTraceId(), "t-") = 1, TRUE)
...
Scenario("LIVE: a served request has measured latency and an on-wire trace")
	...
	Then("the last request carries a trace id", StzFindFirst($oLive.LastTraceId(), "t-") = 1, TRUE)
	Then("the served trace names the worker endpoint (not 'none')",
		StzFindFirst($oLive.RecentTraces(1)[1][3], "127.0.0.1") = 1, TRUE)
