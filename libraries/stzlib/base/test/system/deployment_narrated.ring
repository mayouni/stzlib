...
chk("the DELIVERY log records the production phase (a rehearsal, no actor yet)", StzFindFirst("rehearsal", oBrain.Log().AsText()) > 0)
...
chk("the deployment LOG captured the failure as an ERROR entry", oDepR.Log().CountOfLevel(:error) >= 1)
chk("...and recorded the rollback as a warning", StzFindFirst("rolling back", oDepR.Log().AsText()) > 0)
chk("...the log is STRUCTURED -- queryable by the failing part", len(oDepR.Log().Where(:part, "api")) >= 1)
...
chk("a committed run LOGS its steps done -- no errors, run complete", oDepA.Log().CountOfLevel(:error) = 0 and StzFindFirst("run complete", oDepA.Log().AsText()) > 0)
chk("...renderable as JSON for any log pipeline (structured entries)", StzFindFirst(char(34) + "op" + char(34) + ": " + char(34) + "store", oDepA.Log().AsJson()) > 0)
...
oEmu.CompileEngineQ(FALSE)   # fast bundle: the map + shell, without the per-part Zig compiles
oEmu.SetOutDirQ(cBase + "/emubundle")
oEmu.Build()
...
oSlEmu.CompileEngineQ(FALSE)
oSlEmu.SetOutDirQ(cBase + "/slbundle")
oSlEmu.Build()
