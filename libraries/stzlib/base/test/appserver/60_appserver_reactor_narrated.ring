...
Scenario("a WEB route serves a request end-to-end")
	...
	Then("the client got a 200", StzFindFirst("200 OK", cBody) > 0, TRUE)
	Then("with the query-driven body", StzFindFirst("hello softanza", cBody) > 0, TRUE)
	Then("and a Content-Length header", StzFindFirst("Content-Length:", cBody) > 0, TRUE)
EndScenario()
...
Scenario("a POST route sees the framed body")
	...
	oSrv.ServeOne(3000)
	cBody = oClient.AwaitTcp(nJob, 5000)
	Then("the body made the round trip", StzFindFirst("echo:ping-42", cBody) > 0, TRUE)
EndScenario()
...
Scenario("the built-in /health probe answers without any user route")
	...
	oSrv.ServeOne(3000)
	cBody = oClient.AwaitTcp(nJob, 5000)
	Then("health reports 200", StzFindFirst("200 OK", cBody) > 0, TRUE)
	Then("as json with a status field", StzFindFirst('"status":"healthy"', cBody) > 0, TRUE)
EndScenario()
...
Scenario("MBaaS floor: an exposed table serves REST over sqlite")
	...
	oSrv.ServeOne(3000)
	cBody = oClient.AwaitTcp(nJob, 5000)
	Then("the insert is confirmed with 201", StzFindFirst("201 Created", cBody) > 0, TRUE)
	...
	oSrv.ServeOne(3000)
	cBody = oClient.AwaitTcp(nJob, 5000)
	Then("the rows come back as json", StzFindFirst('"rows":[["probe-1","21.5"]]', cBody) > 0, TRUE)
	...
	oSrv.ServeOne(3000)
	cBody = oClient.AwaitTcp(nJob, 5000)
	Then("the count is numeric json", StzFindFirst('"count":1', cBody) > 0, TRUE)
EndScenario()
...
Scenario("MBaaS full CRUD: read / update / delete ONE row by id")
	...
	Then("it is created (201) and sqlite auto-assigned id=1",
		StzFindFirst("201 Created", cBody) > 0 and $oDb.Value("SELECT id FROM item") = "1", TRUE)
	...
	Then("the single row comes back keyed as {row}",
		StzFindFirst('"row":["1","widget","5"]', cBody) > 0, TRUE)
	...
	oSrv.ServeOne(3000)
	cBody = oClient.AwaitTcp(nJob, 5000)
	Then("one row was updated", StzFindFirst('"updated":1', cBody) > 0, TRUE)
	...
	oSrv.ServeOne(3000)
	cBody = oClient.AwaitTcp(nJob, 5000)
	Then("an honest 404 for a missing row", StzFindFirst("404 Not Found", cBody) > 0, TRUE)
	...
	oSrv.ServeOne(3000)
	cBody = oClient.AwaitTcp(nJob, 5000)
	Then("one row was deleted", StzFindFirst('"deleted":1', cBody) > 0, TRUE)
