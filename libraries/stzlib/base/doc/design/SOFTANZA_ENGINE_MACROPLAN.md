...
| 2026-06-14 | 75      | M-RX2      | **HTTP TEST-HARDENING -- offline loopback regression net.** Addressed the honest gap that the HTTP suite was acceptance/smoke testing against the live internet (fragile, NOT CI-runnable, conflates outages with regressions; per the standing rule live-net tests run outside CI). Built a local loopback HTTP server as a test fixture: new DLL **stz_testsrv** (engine/src/testserver.zig) -- a std.net HTTP/1.1 server on 127.0.0.1, one detached thread per connection (so a /slow handler doesn't block others), raw `std.posix.recv`/`send` on the socket handle (std.net.Stream.read/writeAll hit Windows error 87 on sockets -- gotcha), canned responses by path: /status/{n}, /headers, /echo, /big/{n}, /slow/{ms}, /redirect/{n}, /auth, /setcookie, /checkcookie. Bridge StzEngineTestServerStart(port)->actualPort / Stop; start/stop unblocks the blocked accept() via a self-connection. New narrated **73_http_loopback 15/15** exercises the integration seam (curlcore/http.zig/applyOpts/stzHttpClient) DETERMINISTICALLY and OFFLINE -- exact non-2xx (404/500/204), response-header capture, POST echo, 100KB body intact, the overflow (-2) path with a 5MB body, redirect followed vs FollowRedirects(FALSE)=302, request-timeout firing on /slow, auth 401-without/200-with, and cookie round-trip via a jar file. This converts the manual internet-dependent smoke into an automated, CI-able regression net for the HTTP layer (the live suites 52/71/72 stay as real-world acceptance). Scope note: we test OUR seam, not the vendored libs' internals (curl/zlib/nghttp2 are battle-tested upstream). zig build + zig build test clean; full network suite (52-73) + reactive green. DLLs 90->91, regs 1082->1084, last_session 75. |
| 2026-06-14 | 76      | M-S2       | **Classic->narrated test conversion resumed (run-for-real + fix).** Every narrated suite is EXECUTED and its errors fixed; randomness scenarios run 6-8x to confirm no flakiness; random ops assert invariants (length/range/uniqueness) NOT the stale one-off `#-->` sample values. Domain 1 -- **listofnumbers/00_listofnumbers_narrated 28/28** (stats, NSmallest/NLargest, filters, Diffs, Steps, Nearest, Neighbors, sign-mix, global reducers, 50-draw random invariants). Surfaced + fixed a real LATENT BUG: NSmallestNumbers/NLargestNumbers chained `.SortUpQ()`/`.FirstNItems()` (not stzList methods -> R14) -> `.SortInAscendingQ()`/`.Section(1,n)`/`.LastNItems(n)`; classic 13 now clean. Domain 2 -- **uuid/00_uuid_narrated 17/17** (v4 format/version/variant/validity, nil UUID, WithoutHyphens, + 500-generation uniqueness invariant, run 6x stable). Helper-func construction (NumsOf) dodges the C27 inline-new-in-Scenario trap; LON collided with stzSmallFuncs LoN (case-insensitive) -> renamed. last_session 76. M-S2 stays partial (incremental, many domains remain). |
| 2026-06-14 | 77      | M-S2       | **Classic->narrated conversion grind, domain by domain (run-for-real).** 6 more domains, each executed + fixed (randomness run 5-8x): counter/00 9/9 (cyclic counter; stzIntSeq Count()/First()/At() -- classic .Len() was stale), random/00 7/7 (ARandomNumberBetween/random01 bounds, Some() subset, Randomize permutation + actually-shuffles invariants), ordinal/00 12/12 (en st/nd/rd/th incl teens + fr 1er/2e), octalnumber/00 7/7 (stale formats fixed: ctor now needs 0o prefix, FromHex/FromBinary need 0x/0b), decimaltobinary/00 4/4, binarynumber/00 5/5. **Skipped plural/singular** -- their morphological rule pipeline is incomplete (Plural("bus")!="buses", Singular("buses")="busess"); that's a function defect / feature work, not test hardening, and writing tests would lock in broken behavior (flagged, not converted). Arc total this M-S2 push: 8 domains converted (listofnumbers, uuid, counter, random, ordinal, octalnumber, decimaltobinary, binarynumber) + 1 latent bug fixed (stzListOfNumbers NSmallest/NLargest). last_session 77. M-S2 still partial (many domains remain). |
| 2026-06-14 | 78      | M-S2       | **Classic->narrated grind continues (run-for-real + flag defects).** 3 more domains, each executed until green: duration/00 28/28 (stzDuration -- seconds + natural-language ctor, Days/Hours/Minutes/Seconds + TotalX components, arithmetic +/-/*// with numbers/strings/duration objs, comparisons via operators + IsLessThan/IsGreaterThan/IsEqualTo/IsBetween, ToString/ToStringXT/ToHuman/ToCompact/ToSimple, AddMinutes/AddHours/Subtract mutators), hashlist/00 18/18 (global safe-access HasKey/HasKeys/HasKeysXT/HasPath flat+nested + stzHashList NumberOfPairs/Keys/Values/[:key]/NthKey/ContainsKey/FindValue/FindValues + named hashlist Name/StzType), listofstrings/00 10/10 (ConcatenatedUsing+empty, NumberOfStrings, Contains/Find, ToUpperQ, Reversed, SortInAscendingQ, RemoveDuplicates). **Flagged 2 real latent defects instead of locking broken behavior** (spawn_task chips): (1) stzListOfBytes NumberOfBytes()/Bytecodes()/Bytes() use codepoint-length (StzLen) where raw byte-length (len) is needed -> undercount/truncate on multibyte UTF-8 ("mZh" 6 bytes reports 3), while NLeftBytes/Range use byte-based StzLeft/StzMid (correct) -- asymmetry; (2) stzListOfStrings.SortBy prepends a spurious empty [] element (order correct). Helper-naming gotcha: LoS collides with stzSmallFuncs LoS (case-insensitive C22) -> renamed Sl. Q-mutation gotcha: ToUpperQ mutates in place, so chained Reversed saw uppercased data -> use fresh objects per assertion. Arc total: 11 domains converted + 1 latent bug fixed + 2 defects flagged. last_session 78. M-S2 still partial. |
| 2026-06-14 | 79      | M-S2       | **Classic->narrated grind continues + 2nd latent bug fixed.** 2 more domains, each run until green: list2d/00 6/6 (stzList2D -- Content() verbatim, Transposed() non-mutating with source-intact check, TransposeQ() in-place mutation, involution invariant transpose-twice==original, global Transpose() parity), listofchars/00 7/7 (stzListOfChars -- codepoint construction [945,946,947] + range construction StzListOfCharsQ("A":"E"), NumberOfChars, Unicodes round-trip, NumberOfUniqueChars; asserts on codepoints not glyphs so console stays ASCII-clean per the no-non-ASCII-output rule). **Fixed a real latent bug**: stzStringCharList.Concatenated() (libraries/stzlib/base/string/stzStringCharList.ring) looped `for i = 1 to nLen` but the initialised length var is `_nLen_` -> R24 uninitialized-variable crash; this broke NumberOfUniqueChars (routes through Concatenated). One-char fix nLen->_nLen_ (same class of bug as the listofnumbers Sort fix). Helper-naming gotcha continues: Loc/Lc/LC all collide with stzSmallFuncs (case-insensitive C22) -> named Lchr. Arc total: 13 domains converted + 2 latent bugs fixed + 2 defects flagged (stzListOfBytes byte/codepoint conflation, stzListOfStrings.SortBy spurious empty element). last_session 79. M-S2 still partial. |
| 2026-06-14 | 80      | M-S2       | **Landed the 2 previously-flagged latent defects + 3 more domains.** The two fixes were already sitting uncommitted in the working tree from the prior session; verified end-to-end and committed: (1) stzListOfBytes Bytecodes()/NumberOfBytes() StzLen(@cData)->len(@cData) -- byte methods now count BYTES not codepoints (NumberOfBytes("mZh")=6 was 3; Bytecodes lists all bytes; Bytes() one entry per byte); (2) stzStringList.SortBy rebuilt 'Content()+[]' (which nests an empty list that sorts to front on key len([])=0) as an explicit shallow-copy loop -- spurious leading [] gone. New/grown suites: listofbytes/00 11/11 (byte counts ASCII vs multibyte, Bytecodes, NumberOfBytesPerChar, ToHex 0x-prefix, byte-accurate slicing -- asserts byte codes/counts never glyphs), listofstrings/00 -> 12/12 (+2 SortBy assertions), listoflists/00 11/11 (jagged Sizes/MinSize/MaxSize/HowManyMissing/FindMissing/NthList/NthCol/@IsListOfLists; classic Sizes [4,2,2] stale -> [4,3,2]), listoftimelines/00 9/9 (stzTimeLines GlobalStart/End/Duration/DurationQ.ToHuman/NumberOfLanes/Lanes-uppercased + date-only From auto-00:00:00). Arc total: 16 domains converted + 2 latent bugs fixed (both flagged defects resolved). last_session 80. M-S2 still partial. |
| 2026-06-14 | 81      | M-S2       | **Grind continues + 2 more latent bugs fixed (4 total this arc).** hexnumbert/00_hexnumber 12/12 (HexPrefixes/RepresentsNumberInHexForm/HexToDecimal helpers; stzHexNumber Content/WithPrefix/ToDecimal/ToBinary/ToOctal; FromDecimal round-trip; ctor now needs canonical 0x prefix like octalnumber). **Fix**: stzHexNumber FromDecimalForm/FromOctalForm/FromBinaryForm set @cHexNumber = StzXQ(n).ToHexForm() which returns the PREFIXED form ("0x167A"), but @cHexNumber must hold prefix-less digits -> Content() wrongly returned "0x167A" and WithPrefix() doubled to "0x0x167A"; added _StripHexPrefix() (mirrors init's prefix stripping) and routed all three through it. char/00_char 18/18 (stzChar/stzStringChar: name<->codepoint round-trip via Unicode NAMES, case mapping Lowercased/Uppercased/IsUppercase/IsLowercase, category predicates IsLetter/IsDigit/IsSpace/IsPunctuation, IsVowel; driven by codepoints/names to keep console ASCII). **Fix**: stzStringChar.IsVowel called Vowels() which resolves to the inherited stzString.Vowels() (vowels CONTAINED in content) -> returns [] for a single char -> IsVowel ALWAYS returned FALSE (even 'a'); rewrote to test the codepoint against the ASCII vowel set [65,69,73,79,85,97,101,105,111,117]. Arc total: 18 domains converted + 4 latent bugs fixed. last_session 81. M-S2 still partial. |
| 2026-06-14 | 82      | M-S2       | **Grind continues + 5th latent bug fixed.** grid/00_grid 13/13 (stzGrid: NumberOfRows/Columns/Cells; relative MoveUp/Down/Left/Right; edge clamping; absolute MoveToCell/MoveToFirstNode/MoveToLastNode; CurrentPosition is [col,row]). **Fix**: stzGrid.MoveToCell(nCol,nRow) was declared with an EMPTY body so calls silently did nothing (sibling alias Moveto correctly delegated) -> added the missing This.MoveToNode(nCol,nRow). json/00_json 10/10 (IsJsonList/IsJsonString predicates; ListToJson compact emission incl nested objects; JsonToList parsing of pairs/nested/arrays; list->JSON->list round-trip identity). csv/00_csv 7/7 (CSVSeparator default ';' + SetCSVSeparator round-trip with state restored; IsCSV well-formed vs sentence; CSVToList column-major grouping; ListToCSV header+rows). Arc total: 21 domains converted + 5 latent bugs fixed. last_session 82. M-S2 still partial. |
| 2026-06-14 | 83      | M-S2       | **Grind continues.** namedvars/00 6/6 (dynamic named variables: Vr(name)/Vl(value)/v(read)/vxt(name+value)/VrVl(set); computed names in a loop + reassignment). dataset/00 12/12 (stzDataSet descriptive statistics: Mean/Median/Range/Sum/Min/Max/Count asserted exactly; StandardDeviation/Variance/CoVar/GeometricMean/HarmonicMean rounded to 2dp via a Rnd2 helper floor(n*100+0.5)/100 to avoid float-equality flakiness). func/00 24/24 (Softanza predicates: IsNull/IsEmpty are sentinel-aware -- only ''/NullObject() null, ''/[]/NullObject() empty; IsTrue/IsFalse asserted on PRIMITIVES only -- Ring-native truthiness, ''/0/[] falsy; classic IsTrue([])=TRUE was stale). **Flagged 1 defect**: IsTrueXT/IsFalseXT crash (R19) on any object -- StzIsTrueXT's isObject branch calls IsTrueObject()/IsFalseObject()/IsNullObject() WITHOUT passing the object (they need a param); so plain IsTrue (raw Ring `if p`) treats every object as truthy and the sentinel-aware path is unreachable. Arc total: 24 domains converted + 5 latent bugs fixed + 1 flagged. last_session 83. M-S2 still partial. |
| 2026-06-14 | 84      | M-S2       | **Grind continues.** listofhashlists/00 12/12 (stzListOfHashLists: construct/NumberOfItems/IsEmpty/Content/ListOfHashLists + ToListOfStzHashLists yielding stzhashlist objects with key/value access + empty-list edge). orgchart/00 10/10 (stzOrgChart: AddExecutive/Manager/StaffXT + ReportsTo hierarchy, NodeCount/EdgeCount, DirectReports/DirectReportsN, vacancy tracking VacantPositions/VacancyRate, AddPersonXT+Assign(person,:ToNode=pos)+PersonData; objects built outside brace-blocks so Then() stays in file scope). entity/00 9/9 (stzEntity: Name/Type, Property(:key) get, SetProperty modify+add, HasName; avoids the volatile Created()/'created' timestamp). Arc total: 27 domains converted + 5 latent bugs fixed + 1 flagged. last_session 84. M-S2 still partial. |
| 2026-06-14 | 85      | M-S2       | **Grind continues + 6th latent bug fixed.** locale/00 9/9 (stzLocale: C-locale Lowercase/Uppercase + en_US/fr_FR identity Abbreviation/CountryName/LanguageName/CountryNumber, from stzlib's own data tables so deterministic). knowledgegraph/00 11/11 (stzKnowledgeGraph: AddFact/RemoveFact/Facts with lowercased predicates, NodeExists, pattern Query(["?x",:Pred,"O"]) returning bindings + fully-bound existence check, Predicates/Relations of an entity). **Fix**: stzGraph.RemoveThisEdge matched edge from/to RAW, but node ids + edge endpoints are StzLower-normalised and every other stzGraph method (AddEdge, edge-exists) StzLowers its lookup args -> a caller passing original casing removed nothing; surfaced as stzKnowledgeGraph.RemoveFact being a silent no-op. Added StzLower on from/to (idempotent -> already-normalised callers unaffected). **Skipped** linearsolver: its greedy solver returns a clearly suboptimal answer (x=0,y=8 for max 3x+2y; opt is 24 not 16) -- deterministic but not worth locking, and LP-solver verification is out of grind scope (noted, not flagged). Arc total: 29 domains converted + 6 latent bugs fixed + 1 flagged. last_session 85. M-S2 still partial. |
| 2026-06-14 | 86      | M-S2       | **Grind continues.** datawrangler/00 6/6 (stzDataWrangler list cleaning: TrimWhitespace, RemoveDuplicates, NormalizeCase(lower/upper), HandleMissingValues('remove') dropping ''/NULL, plus a full chained pipeline read back via GetData). date/00 15/15 (stzDate from FIXED date strings -- no clock-dependent Today/Tomorrow: components Day(weekday name)/DayNumber/Month(name)/MonthNumber/Year/DayOfYear/IsLeapYear leap+non-leap; arithmetic AddDays with month rollover/AddMonths/AddYears; span+comparison DaysTo/IsBefore/IsAfter). **Flagged**: stzPivotTable intermittent R31 (StzUpper GC self-reference in SetAggregateFunction) on COUNT for certain grouping shapes (single column-group) -- SUM/MAX/MIN/AVG and other shapes fine; skipped pivottable to avoid flaky CI. **Skipped** chainofvalue (Whatever/OnlyWhen/SometimesWhen DSL with print side-effects + intentional randomness -- low deterministic assertability). Arc total: 31 domains converted + 6 latent bugs fixed + 2 flagged. last_session 86. M-S2 still partial. |
| 2026-06-14 | 87      | M-S2       | **Grind continues + 7th latent fix.** datetime/00 13/13 (stzDateTime from FIXED timestamps -- no clock-dependent Now/NowXT: Content/ToEuropean 12h AM-PM/ToEuropeanWithoutAmPm 24h; numeric components Year/Month/Day/Hours/Minutes/Seconds; arithmetic AddHours/AddDays; comparison IsBefore/IsAfter). **Fix**: added missing @StzMid alias to stzPrimitives.ring -- stzMatrex.ParsePattern (+~30 call sites) call @StzMid but only bare StzMid existed -> R3 "Calling Function without definition: @stzmid" crashed stzMatrex at construction. Alias delegates straight through; benefits any @-prefixed caller. **Flagged**: stzMatrex.Match engine broadly broken (size/shape constraints always FALSE on true matches; Rows()/Cols() always TRUE regardless of dimension) -- @StzMid fix only unblocks construction, so no matrex narrated suite until Match is fixed. Arc total: 32 domains converted + 7 latent bugs fixed + 3 flagged (IsTrueXT R19, pivottable R31, matrex Match). last_session 87. M-S2 still partial. |
| 2026-06-14 | 88      | M-S2       | **Grind continues.** html/00 10/10 (stzHtml: CountByTag present/absent, Find/FindFirst + element Text(), Klass/HasClass; targets the live surface -- classic IsValid/HasBody/TagsUsed are gone). listparser/00 10/10 (stzListParser: List/StartPosition/EndPosition/NumberOfSteps/CurrentPosition defaults, Parse(:From/:To/:Step) -> ParsedPositions/ParsedItems/CurrentItem, NextItem cursor advance; built outside brace-blocks so Then() stays in file scope). **Flagged**: stzNumbrex.Match always returns TRUE regardless of property (18 reported prime, 12 perfect, 22 fibonacci) -- same broken-Match family as stzMatrex (number predicates themselves work; it's the Match plumbing). **Skipped** dotcode (SetCode/View rendering side-effects, not deterministically assertable). Arc total: 34 domains converted + 7 latent bugs fixed + 4 flagged (IsTrueXT R19, pivottable R31, matrex Match, numbrex Match). last_session 88. M-S2 still partial. |
| 2026-06-14 | 89      | M-S2       | **Grind continues.** cluster/00 14/14 (stzLoadBalancer: default GetRoutingStats, RegisterCluster/FindCluster, unknown->NULL, same-type re-register replaces without count growth; stzClusterNode: UpdateMetrics + IsOverloaded threshold + GetHealthStatus node_id/cluster_type/load). graphquery/00 6/6 (stzGraphQuery fluent StzGraphQueryQ(g).MatchQ(...).Select("*"): match-all nodes, match by :labeled, match by :where=[:prop,"=",val]; exercises the just-fixed stzGraph). Gotcha re-confirmed: a helper func defined at the TOP of a narrated file orphans the Scenario top-level code (Ring runs main only up to the first func def) -> produced ZERO output; moved BuildGraph() to the bottom. Arc total: 36 domains converted + 7 latent bugs fixed + 4 flagged. last_session 89. M-S2 still partial. |
| 2026-06-14 | 90      | M-S2       | **Grind continues.** graphplanner/00 4/4 (stzGraphPlanner A* over stzGraph: WalkFrom/Minimizing/Execute -> Cost/Route; linear A-B-C cost 20, and a diamond where A* correctly picks A-B-D cost 10 over A-C-D cost 21). i18n/00 7/7 (stzLanguage IsLanguageNumber + code 105->sindhi/pakistan; stzCountry by name France->france/FR and by abbreviation US->united_states/US; lowercased data tables, deterministic). **Flagged**: stzMatrix mutators (Add*/Multiply*) crash R3 -- they call updateList() from RingFastPro which stzBase doesn't load; only read-only Determinant/Size/Diagonal work. **Skipped**: object (truthiness overlaps func; IsAString returns 0, IsEitherA/EachItemIsEitherA arity quirks), naturalmarkup (fragile NLP markup DSL, S1 errors + retired), dotcode (SetCode/View render side-effects). Arc total: 38 domains converted + 7 latent bugs fixed + 5 flagged (IsTrueXT R19, pivottable R31, matrex Match, numbrex Match, matrix updateList). last_session 90. M-S2 still partial. |
| 2026-06-14 | 91      | M-S2       | **Grind continues.** calendar/00 9/9 (stzCalendar from fixed [year,month]: Year/MonthName/TotalDays; SetWorkingDays(Mon-Fri) + IsWorkingDay weekday/weekend + WorkingDays count (Oct 2024=23); AddHoliday/IsHoliday/HolidayName; data methods only, Show() is a visual renderer). misc/00 12/12 (lowercase primitives stzlen/stzleft/stzright; ring_del removes+returns the list vs Ring del->NULL; @substr 3-arg replace / empty-new find-position / numeric slice; @Min/@Max over lists). **Flagged**: English morphology pipeline (Adverb/Plural/Singular share a broken suffix-rule engine -- happy->happyily, gentle->gentlely, basic->asian, bus plural wrong); stzCurrency (Name()/Currency() return a single char -- indexing bug -- and Russia maps to RON instead of RUB). **Skipped**: global (IsStzFindable(Q("ring")) ambiguous vs classic), chardata (data table evolved -- volatile counts). Arc total: 40 domains converted + 7 latent bugs fixed + 7 flagged (IsTrueXT R19, pivottable R31, matrex Match, numbrex Match, matrix updateList, morphology, currency). last_session 91. M-S2 still partial. |
| 2026-06-14 | 92      | M-S2       | **REPAIRED the Match-family defects (2 previously-flagged) via a shared root cause.** Both stzNumbrex and stzMatrex pattern parsers used @StzMid with end-based (start,END) args, but the global @StzMid is COUNT-based (start,count) -- so @StzMid(s,i,i) grabbed i chars not 1 and inner-pattern extraction kept the closing brace; patterns never tokenised. Result: numbrex Match() looped over an empty token list and returned TRUE for everything (18 "prime"); matrex Match() returned FALSE for everything (even 3x3 vs {Size(3x3)}). Fix (both files): route all 40 @StzMid calls each through an end-based helper _Mid(s,n1,n2)=@StzMid(s,n1,n2-n1+1) -- every call site was uniformly end-based in intent, so one sed + one helper per file. numbrex/00 15/15 (Prime/Perfect/Fibonacci/Even/Odd/Square/Composite/Positive, match+non-match each), matrex/00 8/8 (Size exact, mxn any-size, shape(square), match+non-match each). This is the single highest-leverage fix of the arc -- a shared engine bug across the whole {...} pattern-matcher family. Arc total: 42 domains converted + 9 latent bugs fixed + 5 flagged remaining. last_session 92. M-S2 still partial. |
| 2026-06-14 | 93      | M-S2       | **REPAIRED IsTrueXT/IsFalseXT (flagged defect) -- 3 bugs in one branch.** StzIsTrueXT's `but isObject(p)` branch: (1) called IsTrueObject()/IsFalseObject()/IsNullObject() with NO argument -> R19 crash on every object (the sentinel-aware path was unreachable, which is why plain IsTrue treats all objects as truthy) -> pass p; (2) IsStzTrueObject compared classname against "stzfalseobject" (copy-paste from the false predicate) so IsTrueObject answered TRUE for a FalseObject and FALSE for a TrueObject -> "stztrueobject"; (3) the IsNullObject sub-branch returned 1 (true) when NullObjectIsFalse() -> inverted -> return 0. Verified: IsTrueXT(TrueObject)=1, (FalseObject)=0, (NullObject)=0; IsFalseXT(FalseObject)=1, (TrueObject)=0; string/list primitives unchanged (IsTrueXT("Hello")=1, ""=0, []=0); object classic tests 01/02 + func suite stay green. func/00 grew to 32/32 (+ extended-truthiness sentinel scenario). Arc total: 42 domains converted + 12 latent bugs fixed + 4 flagged remaining (pivottable R31, matrix updateList, morphology, currency). last_session 93. M-S2 still partial. |
| 2026-06-15 | 94      | M-S2       | **REPAIRED matrix ENGINE-FIRST (flagged defect) + removed last non-engine dep.** stzMatrix Add*/Multiply* mutators crashed R3 calling RingFastPro's updateList(), which was dropped when all non-engine third-party deps were removed (engine is now the sole backend). Per that direction, did NOT hand-roll a Ring loop -- instead added a NEW engine function stz_matrix_update_region(m, op, r1,r2, c1,c2, val) to engine/src/matrix.zig (add(op=0)/mul(op=1) over a 1-based inclusive rectangle, bounds-clamped), bridged it (ring_UpdateRegion -> StzEngineMatrixUpdateRegion in ring_bridge_matrix.zig), rebuilt the engine (zig build + zig build test clean), and replaced all 11 updateList call sites with a Ring _UpdateRegion() helper (ensure engine matrix -> StzEngineMatrixUpdateRegion -> _SyncFromEngine). matrix/00 narrated 13/13 (dimensions; scalar Add/MultiplyBy; AddInCol/AddInRow; MultiplyCol/MultiplyRow/MultiplyRows; Determinant). matrix was the LAST updateList/RingFastPro user in active Ring code -> that dependency is fully gone. Recorded the engine-first policy in memory (feedback_engine_first_fixes). Arc total: 43 domains converted + 13 latent bugs fixed + 3 flagged remaining (pivottable R31, morphology, currency). last_session 94. M-S2 still partial. |
| 2026-06-15 | 95      | M-S2       | **Grind continues (clean pool thinning).** time/00 12/12 (stzTime from FIXED strings -- no clock-dependent Now: Content/Hours/Minutes/Seconds, HH:mm->:00 normalisation, AddHours/AddMinutes hour-rollover, IsBefore/IsAfter/IsAfternoon/Hours12). tree/00 6/6 (stzTree Nodes ordered, Items flattened leaves, Node(:k), node/item counts, Branches full-path strings). Noted datetime/00 13/13 was already converted (commit 89fc26e5). Checked the in-flight currency worktree fix: it has corrected Russia->russian_ruble/RUB and US->united_states_dollar/USD, but France Abbreviation() still crashes R2 in CountryLocaleAbbreviation -- left to that worktree session rather than risk a conflicting parallel edit. Arc total: 45 domains converted + 13 latent bugs fixed + 3 flagged remaining (pivottable R31, morphology, currency). Clean convertible pool nearly exhausted -- remaining are big core domains (string/list/table/regex) + visual/DSL ones. last_session 95. M-S2 still partial. |
| 2026-06-15 | 96      | M-S2       | **Core-domain suites begin.** regex/00 8/8 (PCRE2-backed stzRegex core: Match presence/absence; AllMatches + NumberOfMatches over multi-match input; character classes [a-z]+; anchors ^...$ + exact {n} quantifiers; capture-group extraction. NOTE: partial-match helpers MatchAsYouType/MatchInProgress return FALSE where the classic expects TRUE -- partial matching looks unwired; left out of the suite, candidate flag). number/00 15/15 (engine stzNumber: Factorial/GCD/LCM/Fibonacci; IsPrime/IsEven/IsOdd/IsMultipleOf; Power/Abs/IsNegative/IsPositive/Digits). These exercise the two most central engine-backed numeric/text modules. Arc total: 47 domains converted + 13 latent bugs fixed + 3 flagged remaining. last_session 96. M-S2 still partial. |
| 2026-06-15 | 97      | M-S2       | **Core giants: representative suites for string, list, table + a 14th latent fix.** string/00 14/14 (stzString engine core: Uppercased/Lowercased; codepoint-correct NumberOfChars incl multibyte 'héllo'=5; Reversed/Contains/FindFirst; Replaced/Split; Trimmed/IsEqualTo/FirstNChars/LastNChars). list/00 16/16 (stzList core: NumberOfItems/Contains/Find-positions/NumberOfOccurrences; Reversed/Sorted/RemoveDuplicates; AddQ/FirstItem/LastItem/Section; Sum/IsEmpty). table/00 12/12 (stzTable: NumberOfRows/Cols/ColNames; Col/Cell/Row; HasCol/HasColNames; Copy+RemoveCol independence). **Fix**: stzTable.HasColumName lowercased the query then did a case-SENSITIVE ColNamesQ().Contains against the as-stored (upper) names -> HasCol/HasColName/HasColNames always returned FALSE for existing columns; fixed to ContainsCS(pcName, FALSE). These suites are representative cores (the three domains carry ~970/670/245 classic blocks) -- the run-for-real safety net for the most central engine-backed text/list/table operations. Arc total: 50 domains converted + 14 latent bugs fixed + 3 flagged remaining (pivottable R31, morphology, currency). last_session 97. M-S2 still partial. |
| 2026-06-15 | 98      | M-S2       | **graph core suite + 15th latent fix.** graph/00 9/9 (stzGraph directed: NumberOfNodes/NumberOfEdges/NodesIds [lowercased]; Neighbors incl case-insensitive query + sink node; ShortestPath a-b-c; IsConnected; ConnectedComponents single component). **Fix**: stzGraph.Neighbors()'s pure-Ring fallback compared the raw pcNodeId against edge from/to which are StzLower-normalised -> Neighbors("A") returned [] while Neighbors("a") returned [b,d]; lowercased the query in the fallback (the engine path already did). Same id-case family as the RemoveThisEdge fix (session 85). Existing graph classic test stays green. Arc total: 51 domains converted + 15 latent bugs fixed + 3 flagged remaining (pivottable R31, morphology, currency). last_session 98. M-S2 still partial. |
| 2026-06-15 | 99      | M-S2/M-E3  | **STRATEGIC: activated the Zig graph engine backend for stzGraph.** stzGraph is foundational (knowledgegraph/graphquery/graphplanner/orgchart build on it) and already had full engine wiring (Neighbors/ShortestPath/PathExists/HasCycle/Reachable/TopologicalSort/In+OutDegree/components via stz_graph.dll's stz_graph_* fns) -- but _EngineAvailable() probed isFunction('stzenginegraphcreate'), and Ring's isFunction() does NOT see natively-registered (ringlib_init) bridge functions, so it ALWAYS returned FALSE and every graph op silently ran the slower pure-Ring fallbacks. Fix: detect via the loader handle isPointer($pStzGraphHandle). Activation then exposed a separator mismatch -- engine stz_graph_neighbors/shortest_path/reachable emitted COMMA-separated lists while the Ring _SplitNewline helper (and the TopologicalSort bridge) use NEWLINE -> standardized those 3 engine fns to '\n'. Now the strategic graph ops run in-engine. Verified: graph/00 10/10 (now asserts _EnsureEngine()=TRUE so a future regression is caught), knowledgegraph/graphquery/graphplanner suites + graph classic tests all green; engine zig build + zig build test clean. Repo-wide scan found NO other isFunction("stzengine...) dead-engine-path anti-patterns (matrix etc. call the engine directly). Arc: 51 domains converted + 16 latent bugs fixed + 3 flagged remaining (pivottable R31, morphology, currency). last_session 99. |
| 2026-06-15 | 100     | M-E3       | **DEEPENED the Zig graph engine (strategic class) + wired to stzGraph.** Added powerful primitives to engine/src/graph.zig with Zig tests: stz_graph_bfs / stz_graph_dfs (explicit BFS/DFS visit order), stz_graph_dijkstra + stz_graph_dijkstra_distance (weighted shortest path + total weight; non-negative weights, -1 if unreachable), stz_graph_is_bipartite (2-colourability). Bridged each (ring_BFS/DFS/Dijkstra/DijkstraDistance/IsBipartite). stzGraph._EnsureEngine now passes a REAL edge weight from the edge :weight property (default 1.0) so Dijkstra reflects weighted edges not hop count. New engine-backed Ring methods: BFS/BreadthFirst, DFS/DepthFirst, IsBipartite, WeightedShortestPath/DijkstraPath, WeightedShortestPathLength/DijkstraDistance. Verified: engine zig build test 1598/1598 green; graph/00 16/16 (BFS a-b-c-d, DFS a-b-d-c, bipartite path=TRUE/triangle=FALSE, Dijkstra picks cheaper a-b-d cost 10 over the 21 route); knowledgegraph/graphquery/graphplanner suites green. graph engine_fns 18->23, ring_bridge_regs 1084->1089. last_session 100. |
| 2026-06-15 | 101     | M-E3       | **Graph engine deepened further: strongly-connected components + MST.** engine/src/graph.zig: stz_graph_strongly_connected_components (Kosaraju -- iterative post-order finish pass, CSR-built transpose, reverse-order labelling; SCC id per node + count) and stz_graph_mst_weight (Kruskal + path-compressed union-find over the undirected edge set; total MST weight, -1 if empty/disconnected). Bridges: ring_NumberOfSCC, ring_StronglyConnectedComponents (grouped: components newline-separated, nodes comma-separated within), ring_MSTWeight. Ring stzGraph: StronglyConnectedComponents/SCC (-> list of id-groups), NumberOfStronglyConnectedComponents/NumberOfSCC, MSTWeight/MinimumSpanningTreeWeight. Verified: engine zig build test 1600/1600 green; graph/00 20/20 (SCC {a,b,c}+{d} on directed cycle+bridge; MST weight 6 on square+diagonal, -1 disconnected); knowledgegraph/graphquery/graphplanner/orgchart suites green. graph engine_fns 23->26, ring methods 28->33, ring_bridge_regs 1089->1092. last_session 101. |
| 2026-06-15 | 102     | M-E3       | **Graph bridges return ready Ring lists (engine does the looping+splitting); enforced two standing rules.** Per user direction (apply ALWAYS/EVERYWHERE, not just stzGraph): (1) never `for X in` in Ring -- use `nLen=len(..)` once + `for i=1 to nLen`; (2) build collections Zig-side and return a ready Ring list -- never return a delimited string then split it with a Ring loop. Refactored ring_bridge_graph.zig: new retLines() splits the engine '\n' buffer in Zig and emits a Ring list via ring_vm_api_newlist/ring_list_addstring2/ring_vm_api_retlist; Neighbors/ShortestPath/Reachable/BFS/DFS/Dijkstra/TopologicalSort now return ready lists; StronglyConnectedComponents builds a NESTED Ring list (list of node-id lists) directly from SCC labels (no intermediate string). stzGraph: deleted _SplitNewline and all This._SplitNewline(...) wrappers + the for-in in SCC; engine-backed methods just `return StzEngine...()`. Verified graph/00 20/20 + knowledgegraph/graphquery/graphplanner/orgchart green; engine builds clean. Recorded both rules in memory (feedback_engine_first_fixes). last_session 102. |
| 2026-06-15 | 103     | M-E3       | **Graph engine deepened to a full analytics suite (4 slices, each committed + tested).** Slice 1 -- MST edge list (stz_graph_mst_edges, Kruskal), articulation points + bridges (stz_graph_articulation_points/bridges, Tarjan low-link over a deduped undirected CSR built by buildUndirected). Slice 2 -- closeness centrality (BFS reachable/sum-of-distances, historical semantics) + betweenness centrality (stz_graph_betweenness, Brandes; undirected halves). Slice 3 -- k-core core numbers (stz_graph_core_numbers, Batagelj-Zaversnik O(V+E)) + PageRank (stz_graph_pagerank, power iteration d=0.85, dangling mass redistributed). Slice 4 -- A* with a coordinate heuristic (Node gains optional x/y; stz_graph_set_coords; stz_graph_astar with Euclidean/Manhattan/none modes, degrades to Dijkstra without coords). All list/pair-returning bridges build Ring lists Zig-side (retCentralityAll/Of generic helpers; retLines for the A* path). Ring stzGraph: MSTEdges, ArticulationPoints/CutVertices, Bridges/CutEdges, Betweenness/ClosenessCentrality(+All), CoreNumber/CoreNumbers, PageRank/PageRankAll, AStarPath/AStarPathManhattan; _EnsureEngine pushes node :x/:y to the engine. Replaced the old O(V^2*path) pure-Ring Betweenness/Closeness. Verified: engine zig build test green; graph/00 38/38; knowledgegraph/graphquery/graphplanner/orgchart green. graph engine_fns 26->36, ring methods 33->42, ring_bridge_regs (global) ->1498. stzGraphPlanner can now adopt the engine A* (its coord heuristic already keys on :x/:y). last_session 103. |
| 2026-06-15 | 104     | M-E3       | **stzGraphPlanner A* moved onto the engine + loop-audit (Ring-splits-engine-output anti-pattern).** (1) Planner A*: the open/closed-set search left Ring entirely. New engine fns stz_graph_set_edge_weight (override a directed edge weight) + stz_graph_astar_full (A* returning BOTH path and explored/closed order in one search); bridges ring_SetEdgeWeight + ring_AStarPlan -> [route, exploredOrder]; stzGraph SetEdgeWeight/AStarPlan. stzGraphPlanner._AStar now pushes each edge's per-optimisation transition cost into the engine as its weight (the cost model is Ring-side data), runs the engine A* (mode 0 = Dijkstra/UCS -> optimal for any non-negative cost; the coordinate heuristic isn't admissible vs arbitrary cost units), then rebuilds [actions, totalCost, route, explanation] + explainability data (nodes_explored/efficiency/alternatives) from the engine's route+explored. _GoalSearch stays Ring-side (goal is a Ring predicate). graphplanner/00 7/7 (+explainability scenario); all planner example tests clean. (2) Loop-audit: swept ALL ring_bridge_*.zig for the 'bridge returns a delimited string, Ring splits it in a loop' anti-pattern. Fixed list (FindAllCS/FindAllW/FindDuplicatesCS/FindNonDuplicatedCS -> ready position lists via retPositions/retPositionsFromList; callers stzList.FindAllItemsW, stzListDuplicates, stzListExtractor stripped of StzSplit) and table (FindCellStringCS -> nested [col,row] list; FindInColStringCS -> row list; stzTableSearch.FindCellCS reduced to one engine call). Also confirmed ZERO real `for X in` iterator loops remain in active base source (all 13 grep hits are string literals / comments). Verified: engine zig build+test green; list/00 16/16, listofnumbers 28/28, listofstrings 12/12, table/00 12/12, table search-delegation 6/6, graph/00 38/38, knowledgegraph/graphquery/orgchart green. graph engine_fns 36->38, ring methods 42->44, ring_bridge_regs ->1500. last_session 104. |
| 2026-06-15 | 105     | M-S2       | **Phase 1: closed ALL remaining flagged defects (3).** (1) pivottable R31 (StzUpper GC self-ref on COUNT) no longer reproduces -- resolved by the earlier @StzMid/GC work; locked in with pivottable/00 9/9 (COUNT/SUM/AVG/MIN/MAX + the previously-crashing COUNT-with-totals path; stable across 5 runs). (2) stzCurrency: Currency()/Name() returned a single CHAR (code indexed currency-name[7][1] = first char; col layout is [7]=name,[8]=fractional unit,[9]=base) -> now full names; and Abbreviation() crashed R2 for shared currencies (France & Germany both euro) because Country() reverse-mapped currency->country ambiguously -> init() now stores @cCountry@ and Country() returns it when present. currency/00 16/16. (3) plural/singular/adverb morphology: the rule engine mapped the \\1 placeholder to CapturedValues()[1] (the FULL match) instead of [2] (first capture group), so city->cityies, happy->happyily; fixed to map \\j -> aCaptured[j+1] in stzPlural/stzSingular/stzAdverb. Adverb also had unanchored geographic rules ((asia?) matched the 'asi' in 'basic' -> 'asian'); anchored to ^...$ and normalised string '4' priorities to numeric. plural/00 12/12, singular/00 7/7, adverb/00 13/13. KEY: stzRegex.CapturedValues() = [fullMatch, group1, ...]. No open flagged defects remain. last_session 105. |
| 2026-06-15 | 106     | M-E3       | **Phase 2 (loop-audit deeper) + Phase 3 (engine deepening).** Phase 2: confirmed ZERO real `for X in` loops in active base source (all grep hits are strings/comments) and that the remaining core-class Ring loops are either deliberate workarounds, niche, or would PESSIMIZE if moved (a list predicate that short-circuits beats a full marshal into the engine). One solid win: stzList.FindDuplicates() was an O(n^2) Ring nested loop using `=` which can't compare sublists (silently returned [] for lists of lists) -> routed through the proven engine StzEngineListFindDuplicatesCS (O(n) hashing) with a Ring fallback; nested-list items now correct ([[1,2],[3],[1,2],[3]] -> [3,4]). list/00 18/18. Phase 3: stzString.ReplaceCS delegated to the engine. It had a byte-oriented Ring substr-walk fallback (kept because the engine replace once @memcpy-panicked on Windows) that CORRUPTED multibyte content. The engine str_replace_cs was since rewritten to build a fresh buffer (no aliasing) and is codepoint-aware; verified across ASCII/case-insensitive/multibyte/60 length-combos with no panic. ReplaceCS now codepoint-correct (naive=naïve.Replaced('ï','i')); string/00 16/16; Replace classic tests clean; stale CLAUDE.md note updated. Remaining byte-oriented substr() methods (Vowels/FindNumbersAsSections/section-upper/contains-at) logged in memory as UTF-8 follow-ups. last_session 106. |
| 2026-06-15 | 107     | M-S2       | **Phase 4: more narrated conversions; surfaced + fixed 2 engine bugs.** sortedlist/00 8/8 -- converting it exposed that stzSortedList.Add() was a SILENT NO-OP: it passed a StzValue handle minted by stz_value.dll into StzEngineListSortedInsert in stz_list.dll, but ring_api handle tables are per-DLL so the handle didn't resolve -> 'switch on corrupt value' panic / no insert. Fixed by adding type-specific stz_list_sorted_insert_{int,float,string} that build the value INSIDE the list DLL (shared sortedInsertOwned helper) + rewiring Add(). Second bug: StzValue.compare() ordered mixed int/float by tag enum, not value, so a float inserted among ints landed last ([3,1].Add(2.5) -> [1,3,2.5]); compare() now does cross-type numeric comparison -> [1,2.5,3]. yielder/00 8/8 (Map :Abs/:Negate/:Double/:Square, Filter :IsPositive/:IsNegative/:IsEven, Reduce :Sum -- no defects). Engine zig build+test green; list/listofnumbers/listofstrings/table/dataset/sortedlist suites green (compare() is shared by sort/dedup). Recorded the cross-DLL handle trap in memory. last_session 107. |
| 2026-06-15 | 108     | M-E3       | **Phase 5: codepoint-correct stzString section ops (UTF-8 follow-ups).** Two general-purpose section methods took codepoint positions but mixed byte-oriented Ring builtins, corrupting multibyte: _UppercaseSubStringRange (upper()->StzUpper, len()->StzLen; 'café'.UppercaseSubStringXT(1,4) had left 'CAFé') and ReplaceInSection (StzMid/substr/len -> _EngineSlice/_EngineSliceFrom/StzLen; 'déjà xx' had been mangled). Now 'CAFÉ x' / 'déjà YY', ASCII unchanged. string/00 19/19 (+3 multibyte section assertions). Confirmed Vowels() is already UTF-8-safe (advances by codepoint byte-length, ASCII vowels only). Number-parsing helpers (NumbersComingAfterCS, FindNumbersAsSections) left as-is (ASCII-digit-oriented, niche). Gotcha recorded: global StzMid() returns '' on multibyte -> use This._EngineSlice inside stzString. last_session 108. |
| 2026-06-15 | 109     | M-E3       | **Phase 5 (cont.): swept stzString's broader byte-oriented surface; fixed 4 more multibyte/correctness bugs.** Classified every substr()/len()/upper()/lower()/right()/left() usage (agent audit) into BUG vs SAFE. Fixed: UppercaseSubString(string-branch) upper()->StzUpper ('café'->'CAFé' had left the accent); RemoveThisTrailingChar (right()/left() byte -> StzLen+_EngineSlice; removing a multibyte trailing char was a silent no-op); RemoveThisLeadingChar (left() byte-compare -> _EngineSlice(1,1)+_EngineSliceFrom(2)); FindNextCS -- the big one: it mixed byte len()/lower() with a StzMidToEnd tail walk and returned 0 even for ASCII (FindNext('abc',2) on 'abcabc' missed pos 4), now delegates to the engine _FindSubStr(sub, nStart+1, case) (codepoint + case-correct). Confirmed SAFE (no change): Vowels (advances by codepoint byte-len), EndsWith/StartsWithANumber (edge-byte-of-multibyte is never an ASCII digit), DiacriticsRemoved (intentional byte NFD parsing), Bytes/Hash (intentionally byte). Left niche/ASCII-only: NumbersComingAfterCS, FindNumbersAsSections, Boxify width. string/00 25/25 (multibyte section/char + FindNext assertions); classic string tests clean. last_session 109. |
| 2026-06-15 | 110     | M-E3       | **Phase 5 mop-up: finished the stzString byte-oriented substr sweep.** Fixed Boxify (bar width len()->StzLen; frame was over-wide on multibyte) and BoundsOfXT (prefix/suffix caps right()/left()/len() -> StzLen + _EngineSlice/_EngineSliceFrom; byte caps split multibyte chars). That closes the general-purpose set (8 methods fixed total across Phase 5: _UppercaseSubStringRange, ReplaceInSection, UppercaseSubString(string), RemoveThis{Trailing,Leading}Char, FindNextCS, Boxify, BoundsOfXT). Remaining substr sites are intentionally byte-oriented + documented: number parsers (NumbersComingAfterCS, FindNumbersAsSections -- ASCII-digit, byte-position by design, no active codepoint-slicing consumer), Bytes/Hash, DiacriticsRemoved (NFD), IsMarquer (ASCII tokens), and Vowels/EndsWith/StartsWithANumber (proven safe). string/00 27/27; Boxify/BoundsOf classic tests clean. last_session 110. |
| 2026-06-15 | 111     | M-S2       | **Phase 6: pointed the UTF-8/byte lens at the char classes.** stzStringCharList.Uppercase/Lowercase mapped each char with byte upper()/lower() -> multibyte chars unchanged ([a,é,b].Uppercase = [A,é,B]); now StzUpper/StzLower -> [A,É,B]. stzStringChar.IsMandarinNumber had a typo This.Conten() -> R14 crash on every call; fixed to This.Content() (now '5'->FALSE, Mandarin one (U+4E00)->TRUE). Verified SAFE (no change): IsAscii/AsciiCode -- Ring's ascii() raises on a multibyte char so the try/catch is correct; stzStringChar construction is codepoint-aware (NumberOfChars==1) and handles 2/3/4-byte chars. char/00 20/20, listofchars/00 9/9 (+multibyte case + Mandarin assertions); char/listofchars classic tests clean. last_session 111. |
| 2026-06-15 | 112     | M-E3       | **Phase 7: stzListOfStrings + stzStringText UTF-8 audit -- 3 real multibyte bugs.** Both classes were already codepoint-clean (StzUpper/Lower/Left/Right/Len throughout) but probing through them surfaced engine/primitive bugs: (1) engine str_extract_words treated only ASCII [a-zA-Z'] as word chars -> SPLIT accented words ('café école naïve' -> caf/cole/na/ve) and disagreed with NumberOfWords; now any byte >= 0x80 is a word char (isWordByte) so words stay intact, ASCII punctuation still separates. (2) stzString.Chars() round-tripped a NUL-delimited engine buffer through Ring _SplitNullDelimited, dropping trailing multibyte ('café'->c/a/f) and intermittently R31-crashing on multibyte; added engine bridge ring_StringCharsList that builds the Ring char list Zig-side (UTF-8 iteration, correct for 2/3/4-byte incl emoji), Chars() now returns it. (3) _SplitNullDelimited (still used by ~5 methods) walked StzMid(buf,i,1) -- codepoint slice indexed by a byte counter -- rewritten to byte-range substr() per NUL segment. NOTE: char(135)-style test strings are codepage-mangled on Windows (Ç dropped) -- build multibyte test data from real UTF-8 literals or clean char-concats (é/ï work). string/00 29/29, ttext/00 12/12 NEW; engine zig build test green. last_session 112. |
| 2026-06-15 | 113     | M-E3       | **Phase 7 (cont.): string helper-class byte-op sweep.** stzStringFinder.StartsWithAnyCS/EndsWithAnyCS did byte-oriented prefix/suffix matching (substr / StzMidToEnd-by-byte-position + lower()) -> StartsWithAny CI failed on multibyte and EndsWithAny failed on a multibyte suffix even case-sensitively ('café'.EndsWithAny(['fé'])=0). Both now delegate per-item to the engine-backed @oString.StartsWithCS/EndsWithCS. string/00 31/31. Classified as intentional/niche (left as-is, documented): stzStringReplacer.ReplaceByManyCS (byte-position-consistent scan+apply; case-sensitive correct incl multibyte, only CI-multibyte limited -- a correct fix needs codepoint position remapping since casefolding changes byte lengths), and stzStringFunc.@substr (deliberate byte-substr helper). last_session 113. |
| 2026-06-15 | 114     | M-E3       | **Phase 7 (cont.): extended the byte/codepoint lens beyond the string dir.** Repo-wide byte-op density scan; fixed two real multibyte case-insensitivity bugs: stzTable.FindInColCS used byte upper() for CI cell matching (substring + exact) -> accented cells didn't fold case; now StzCaseFold. stzKnowledgeGraph normalised triple terms + Predicates/Relations/SimilarTo entity queries with byte lower() -> a 'CafÉ' fact didn't match a 'café' query; now StzLower (consistent with the StzLower-based graph node-id storage). table/00 14/14, knowledgegraph/00 12/12 (each +multibyte assertions); classic FindInCol + graphquery green. Flagged stray unused stznatural-copy.ring (not loaded) for deletion. Classified intentional/niche (left): @substr, ReplaceByManyCS CI-multibyte, number parsers. last_session 114. |
| 2026-06-15 | 115     | M-E3       | **UTF-8/byte lens sweep COMPLETE across the codebase.** Final pass over the lower-yield classes: fixed stzSmallFuncs.CenterText (byte len()/left() -> StzLen/StzLeft; visible-column padding + safe truncation; used by pivot-table-show). Swept stzTimex / stzNetwork / stzUnicodeData / stzJsonFuncs / stzYielder / stzTablex / stzListShow / stzListMover and confirmed all remaining byte ops are SAFE (ASCII-domain data: hex/JSON-structure/IP/duration specifiers; param keywords; or correct intentional UTF-8 byte-structure analysis). Cumulative across the lens thread (sessions 105-115): ~18 multibyte/correctness bugs fixed across stzString, stzList, char classes, string/text-list classes, string helpers, stzTable, stzKnowledgeGraph, stzSmallFuncs -- several in shared engine/primitive code (StzValue.compare, str_extract_words, Chars/CharsList, _SplitNullDelimited, ReplaceCS), each locked in with run-for-real assertions. misc/00 15/15. last_session 115. |
| 2026-06-16 | 116     | M-S2       | **Phase 8: more narrated conversions.** splitter/00 7/7 NEW (stzSplitter: SplitAt position incl head/tail edges, SplitAt[positions], SplitAtSection, SplitAtSections over 1..N -- no defects). Probed several other candidates: timeline (stzTimeLine.End() is unparseable at statement scope -- 'End' is a hard Ring keyword, so narrated Then(...) assertions can't call it; skipped), object (truthiness, redundant with func/00), substring (all tests retired). Flagged stzListex (list-pattern DSL): [@S] single-token pattern crashes R31 in its class-local stzmid()/parsePattern, and [@N,@S] matches ["x",5] (element type/order not enforced) -- deep DSL defect, spawned a task rather than lock in broken behaviour. last_session 116. |
| 2026-06-16 | 117     | M-E3       | **Engine deepening: all-pairs graph distance metrics.** Added eccAndStats (one BFS per source over out-edges -> per-node eccentricity + global sum/count of reachable ordered-pair distances) and exposed stz_graph_eccentricities / diameter / radius / average_path_length (+ Zig tests). Bridges: Diameter/Radius/AveragePathLength + EccentricitiesAll/EccentricityOf. Ring stzGraph.Diameter and AveragePathLength were O(V^2 * BFS) pure-Ring double loops calling ShortestPathLength -> now single engine calls; added Radius/Eccentricity(node)/Eccentricities. graph/00 44/44 (path A-B-C-D-E: diameter 4, radius 1, ecc A=4/C=2, avg 2.0); dependents green. graph engine_fns 38->42, ring methods 44->49, ring_bridge_regs 1500->1509. last_session 117. |
| 2026-06-16 | 118     | M-E3       | **Engine deepening: local clustering coefficient.** stz_graph_clustering fills per-node local clustering (edges among a node's undirected neighbours / k*(k-1)/2) via the deduped undirected CSR + a reused membership bitmap; Zig test green. Bridges ClusteringAll/ClusteringOf. Ring stzGraph.ClusteringCoefficient was an O(k^2) pure-Ring EdgeExists double loop -> now a single engine call; added ClusteringCoefficients. graph/00 48/48 (triangle+pendant: B=1, A=1/3, D=0); dependents + clustering classic tests green. graph engine_fns 42->43, ring methods 49->51, ring_bridge_regs 1509->1511. last_session 118. |
| 2026-06-16 | 119     | M-E3       | **Engine deepening: max-flow / min-cut (NEW capability, not a Ring-loop replacement).** stz_graph_max_flow = Edmonds-Karp (BFS augmenting paths over an n*n residual-capacity matrix built from edge :weight capacities); stz_graph_min_cut returns the saturated cut edges (original edges from the residual-reachable source side S to its complement). Shared buildCapacity/residualBfs helpers; Zig tests green. Bridges MaxFlow + MinCut ([from,to] pairs Zig-side). Ring stzGraph MaxFlow/MaximumFlow -> value, MinCut/MinimumCut -> cut-edge list. graph/00 50/50 (MaxFlow(s,t)=5; cut [s,a],[s,b] sum 5); dependents green. graph engine_fns 43->45, ring methods 51->55, ring_bridge_regs 1511->1513. stzGraph engine is now a comprehensive network-analysis backend (traversal, weighted+A* shortest paths, connectivity, MST, centrality, distance metrics, clustering, flow). last_session 119. |
| 2026-06-16 | 120     | M-E3       | **Engine deepening: community detection (Louvain, one level).** stz_graph_communities = single-level Louvain modularity optimisation over the undirected view (greedy node moves by gain k_i_in(c) - Sigma_tot(c)*deg_i/2m, fixed order, ties keep current/smallest id). Label propagation was implemented first but REJECTED -- it collapsed two triangles joined by one bridge into a single community; Louvain separates them. Zig test green. Bridges NumberOfCommunities + Communities (nested name-list). Ring stzGraph Communities/DetectCommunities + NumberOfCommunities. graph/00 52/52 (two bridged triangles -> {a,b,c},{d,e,f}); dependents green. graph engine_fns 45->46, ring methods 55->58, ring_bridge_regs 1513->1515. Next in the 3-algo plan: min-cost-max-flow, then A* weighted-coordinate heuristic plumbing. last_session 120. |
| 2026-06-16 | 121     | M-E3       | **Engine deepening: min-cost max-flow (algo 2 of 3).** Edge gains an optional cost field + stz_graph_set_edge_cost; _EnsureEngine pushes edge :cost. stz_graph_min_cost_max_flow = successive shortest paths (SPFA over a residual edge-list, paired fwd/rev edges with negated reverse cost) augmenting the cheapest path each round; returns max-flow value + writes min total cost. Zig test green. Bridges SetEdgeCost + MinCostMaxFlow ([flow,cost]). Ring stzGraph MinCostMaxFlow/MinCostFlow(source,sink) -> [flowValue,totalCost] (:weight=capacity, :cost=per-unit). graph/00 53/53 (flow 4 / cost 10); dependents green. graph engine_fns 46->48, ring methods 58->60, ring_bridge_regs 1515->1517. Remaining in plan: A* weighted-coordinate heuristic plumbing. last_session 121. |
| 2026-06-16 | 122     | M-E3       | **Engine deepening: A* admissible-scaled coordinate heuristic (algo 3 of 3 -- DONE).** Refactored the A* core into astarRun(scale) + scale-aware heuristicScaled; stz_graph_astar keeps scale 1.0. New stz_graph_astar_weighted scales the coordinate heuristic by admissibleScale = min edge cost-per-distance ratio, keeping it a lower bound when edge weights aren't unit geometric distance (the plain Euclidean/Manhattan heuristic can OVERESTIMATE and return a suboptimal route). Zig test green. Bridge AStarWeighted; Ring AStarPathWeighted(start,goal,mode). graph/00 55/55 (wide coords + cheap edges: plain returns suboptimal s-t, weighted returns optimal s-a-t); dependents green. graph engine_fns 48->49, ring methods 60->61, ring_bridge_regs 1517->1518. The 3-algorithm plan (Louvain community detection, min-cost-max-flow, weighted A*) is complete; stzGraph is now a full network-analysis backend. last_session 122. |
| 2026-06-16 | 123     | M-E3       | **Cleanup: dropped 5 dead Ring-callable graph bridges.** Audit confirmed every graph ALGORITHM has a stzGraph Ring method + a graph/00 scenario (55 assertions). Removed the registered-but-unused wrappers + regs for nodecount/edgecount/hasedge/removeedge/neighborcount (0 Ring callers; the public NumberOfNodes/NumberOfEdges/EdgeExists/RemoveEdge/NeighborCount serve from @aNodes/@aEdges Ring-side). Underlying engine pub fns kept (used internally by other bridges). graph/00 55/55, dependents green, basic-ops + classic graph tests verified. ring_bridge_regs 1518->1513. last_session 123. |
