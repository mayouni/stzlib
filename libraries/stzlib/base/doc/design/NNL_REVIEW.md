...
| Modality (can/must) | DONE (2026-07-12): kind constraints (ConstrainQ...ToBeQ) + CanQ/MustQ modal questions + the QualifiesAsQ gate; AND the archived per-object design GENERALIZED onto stzObject per the author (AddConstraint named rules -- descriptors or placeholder conditions -- VerifyConstraint(s) with Why, ApplyConstraints = the structured cancel, chainable when clean); enforcement-on-update deferred to the single-update-point refactor the author's own TODO names |
| Evidentiality | frontier (needs a runtime neural model) |
