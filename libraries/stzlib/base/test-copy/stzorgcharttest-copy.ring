...
oOrg {
    ...
    AddManagerXT("vp_sales", "VP Sales & Customer Relations")
    AddManagerXT("vp_eng",   "VP Engineering & Digital")
    AddManagerXT("vp_ops",   "VP Operations & Treasury")
    AddManagerXT("dir_risk", "Chief Risk Officer")     # Reports to CEO → BCEAO violation
    AddManagerXT("dir_audit","Director Internal Audit")# Reports to CEO → BCEAO violation

    # 1. Create the missing Board
    AddExecutiveXT("board", "Board of Directors")
    ...
    # Staff and people
    AddPersonXT("jean", "Jean-Baptiste Kouassi")
    AssignPerson("jean", "ceo")

    AddPersonXT("alia", "Alia Soulaymane")
    AssignPerson("alia", "vp_eng")

    AddPersonXT("lawali", "Lawali Oumarou")
    AssignPerson("lawali", "vp_ops")

    AddPersonXT("maimouna", "Maimouna Nadima")
    AssignPerson("maimouna", "dir_audit")
...
pf()
# Executed in 2.11 second(s) in Ring 1.24
...
oOrg {
    ...
    AddManagerXT("vp_sales", "VP Sales")
...
oOrg {
    ...
    AddPersonXT("mahran", "Mahran Abid")
    AssignPerson("mahran", "sales_mgr")
   ...
   #--> [ ] #TODO Is ist correct?
    ...
    SetClusterColor("green") #ERR // See why it does not have an effetc
//    View()

 //   ViewVacant() #TODO // See why "Sales Manager" node is white although it's not vacant
...
oOrg {
    ...
    AddManagerXT("vp_sales", "VP Sales")
    AddManagerXT("vp_ops", "VP Ops")
    AddManagerXT("vp_eng", "VP Eng")
    ...
    AddPerson("mansour")
    AssignPerson("mansour", "ceo")

    ? @@(Vacant())
    # Highlight just one path
    View()
    #--> all the nodes arer white (vacant) except "ceo" (gold)

    ViewPath("ceo", "ops1") # Or Hilight("ceo", "ops1")
    #--> the path is correctly hilighted (maenta+) but the other
    # nodes are not in white although they are vacant!

    #NOTE // The inverse ("ops1", "ceo") won't show the path*
    # So direction matters!

    #TODO //ViewPath() should be abstracted in stzDiagram, right? 
}

pf()
# Executed in 1.63 second(s) in Ring 1.24
...
oOrg {
    ...
    AddManagerXT("vp_sales", "VP Sales")
    AddManagerXT("vp_eng",   "VP Engineering")
    AddManagerXT("vp_ops",   "VP Operations")
    ...
    AddPersonXT("p_vp_sales", "Fatoumata Diatta")
    AssignPerson("p_vp_sales", "vp_sales")

    AddPersonXT("p_warda", "Warda Karima")
    AssignPerson("p_warda", "vp_eng")

    AddPersonXT("p_salem", "Salem Salhi")
    AssignPerson("p_salem", "dev_b")

    AddPersonXT("p_juior", "Amine Dori")
    AssignPerson("p_juior", "junior_b")
...
oOrg {
    ...
    AddManagerXT("dir_audit", "Dir Audit")
    ReportsTo("dir_audit", "board")  # Compliant

    AddManagerXT("dir_ops", "Dir Ops")
    AddManagerXT("dir_treasury", "Dir Treasury")
...
oOrg {
    ...
    AddExecutiveXT("ceo", "CEO")
    AddManagerXT("vp1", "VP1")
    AddManagerXT("vp2", "VP2")
    ReportsTo("vp1", "ceo")
    ReportsTo("vp2", "ceo")
...
oOrg {
    ...
    # Add positions with levels
    AddManagerXT("hr_mgr", "HR Manager")
    ...
    AddManagerXT("it_mgr", "IT Manager")
...
oBank {
	...
	# Executive positions
	AddExecutiveXT("ceo", "Chief Executive Officer")
	AddExecutiveXT("cfo", "Chief Financial Officer")
	AddManagerXT("cro", "Chief Risk Officer")
	AddManagerXT("cao", "Chief Audit Officer")
	
	# Department heads
	AddManagerXT("treasury_head", "Head of Treasury")
	AddManagerXT("ops_head", "Head of Operations")
	AddManagerXT("it_head", "Head of IT")
...
oBank {
	...
	AddExecutiveXT("board", "Board of Directors")
	AddExecutiveXT("ceo", "CEO")
	AddManagerXT("cfo", "CFO")
	AddManagerXT("cro", "Chief Risk Officer")
	AddManagerXT("cto", "CTO")
	AddManagerXT("treasury_head", "Treasury Head")
	AddManagerXT("ops_head", "Operations Head")
...
? "=== COMPLETE FILE FORMAT DEMONSTRATION ===" + NL

# 1. KNOWLEDGE GRAPH
oKnow = new stzKnowledgeGraph("Corporate_KB")
oKnow {
    ImportKnow("txtfiles/company_knowledge.stzknow")
    
    ? "Knowledge Facts:"
    ? @@NL( Facts() )
    
    ? NL + "Query: Who reports to Alice?"
    ? @@( Query(["?x", :ReportsTo, "Alice"]) )
    
    ? NL + "Inferred relationships:"
    ApplyInference()
    ? @@( Query(["?x", :IndirectlyReportsTo, "?z"]) )
}

# 2. ORG CHART with all formats
oBank = new stzOrgChart("Complete_Bank_System")
oBank {
    # Load structure
    LoadStzOrg("txtfiles/bank_structure.stzorg")
    
    # Apply styling
    LoadStyle("txtfiles/corporate_brand.stzstyl")
    
    # Load validation rules
    LoadRuleBase("txtfiles/bceao_banking.stzrulz")
    LoadRuleBase("txtfiles/sox_compliance.stzrulz")
    
    # Link to knowledge graph
    ? NL + "=== Linking OrgChart â†' Knowledge Graph ===" + NL
    ...
    # Simulate
    ? NL + "Running Simulation:"
    aResult = RunSimulation("txtfiles/restructure_2024.stzsim")
    ? @@NL( aResult )
...
# 3. WORKFLOW with org integration
oWorkflow = new stzWorkflow("Loan_Process")
oWorkflow {
    ImportFlow("txtfiles/loan_approval.stzflow")
    LoadStyle("txtfiles/corporate_brand.stzstyl")
    LoadRuleBase("txtfiles/bpm_best_practices.stzrulz")
    ...
    ? @@NL( ValidateXT([:sla, :bottleneck]) )
    
    ViewCriticalPath()
}

# 4. PURE GRAPH analysis
oSupply = new stzGraph("Supply_Network")
oSupply {
    ImportGraf("txtfiles/supply_chain.stzgraf")
...
oBank {
    ...
    # Positions
    AddManagerXT("loan_manager", "Loan Manager")
