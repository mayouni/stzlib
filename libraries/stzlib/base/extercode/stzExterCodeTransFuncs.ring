    ...
    def _transform(obj):
        if isinstance(obj, dict):
            ...
            for key, value in obj.items():
                items.append(f"['+StzChar(39)+'{key}'+StzChar(39)+', {_transform(value)}]")
            return "[" + ", ".join(items) + "]"
  ...
  transform <- function(obj, depth = 0) {
    ...
    if (is.numeric(obj)) {
      ...
      if (grepl("e", str_val, ignore.case = TRUE)) {
        return(sprintf("'+StzChar(39)+'%s'+StzChar(39)+'", str_val))
      }
      return(str_val)
    }
    ...
    function _transform(obj, depth=0)
        ...
        if isa(obj, Dict)
            ...
            for (key, value) in obj
                push!(items, "[\'+StzChar(39)+'$(key)\'+StzChar(39)+', $(_transform(value, depth + 1))]")
            end
            return "[" * join(items, ", ") * "]"
        end
    ...
    function _transform(obj, depth = 0) {
        ...
        if (typeof obj === "object") {
            ...
            for (const [key, value] of Object.entries(obj)) {
                items.push("[' + StzChar(39) + '" + key + "' + StzChar(39) + ', " + _transform(value, depth + 1) + "]");
            }
            return "[" + items.join(", ") + "]";
        }
