# stzPolyglotCodeGraph -- the COMMON GROUND for the tree-sitter code-graph
# family (SOFTANZA_INTELLIGENCE_ARCHITECTURE.md 5.8, polyglot refinement).
#
# A code graph is language-agnostic once source is parsed to the shared line
# protocol (CLASS|/METHOD|/FUNC|/IMPORT|/CALL|). This base owns the model,
...
# CyclicCalls. Each LANGUAGE is a peer subclass that keeps its own identity
# and declares only two things:
#     def Language()       -> the engine grammar token ("python"|"javascript")
#     def FileExtension()  -> the source extension for directory scans (".py")
# ...plus any language-specific extra backends it wants (e.g. Python's
# indentation floor). This is why stzPyCodeGraph and stzJsCodeGraph are
# SIBLINGS, not one under the other.
#
# NOTE: stzCodeGraph (the Ring code-graph) is a related but distinct family
# -- single-inheritance model, its own Ring-source parser -- so it stays its
# own class rather than a child here.
...
class stzPolyglotCodeGraph from stzObject
	...
	def init(pcRootPath)
		@cRoot = pcRootPath
		@oGraph = new stzGraph("polycodegraph")
		@aMethods = []
		@acClasses = []
		@aFunctions = []
		@aImports = []
	...
	def Language()
		stzraise("stzPolyglotCodeGraph is abstract for parsing -- use a language subclass (stzPyCodeGraph / stzJsCodeGraph) that defines Language().")
