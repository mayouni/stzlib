...
class stzDateTime from stzObject
    ...
    def ParseStringDateTime(cDateTime)
        ...
        aCustomFormats = [
            "yyyy-MM-dd hh:mm:ss",
            "yyyy-MM-dd hh:mm:ss.zzz",  # Added for milliseconds with space
            "dd/MM/yyyy hh:mm:ss",
            "MM/dd/yyyy hh:mm:ss",
            "dd-MM-yyyy hh:mm:ss",
            "yyyy-MM-dd hh:mm",
	    ...
	    while i <= len(aParts)
	        if isNumber(0+ aParts[i])
	            ...
	            if i < len(aParts)
	                ...
	                but cUnit = "millsecond" or cUnit = "milliseconds"
    ...
    def IsBefore(poOtherDateTime)
        if isString(poOtherDateTime)
			...
			poOtherDateTime = _oOtherDateTime_
        ok
    ...
    def IsBetween(poStartDateTime, poEndDateTime)
        ...
        if isString(poStartDateTime)
			...
			poStartDateTime = _oStartDateTime_
        ok
...
class stzDateTime from stzObject
	...
	def DurationTo(oOtherDateTime)
	        ...
	        nRemainingSecs = nTotalSecs - (nDays * 86400)
	
	        nHours = floor(nRemainingSecs / 3600)
	        nRemainingSecs = nRemainingSecs - (nHours * 3600)
	
	        nMinutes = floor(nRemainingSecs / 60)
	        nRemainingSecs = nRemainingSecs - (nMinutes * 60)
	
	        nSeconds = floor(nRemainingSecs)
	        nMilliseconds = floor((nRemainingSecs - nSeconds) * 1000)
